...
| **v6.15.0** | 4 Septembre 2026 | **Santé Globale MokNet — onglet Super-Admin : 12 domaines, 52 lignes mesurées, 15 réparations à catalogue fermé, sauvegarde et action dans la même transaction** | Super Admin (1 fichier existant, +25 lignes additives), Supabase (`health_snapshots`, 13 fonctions `health_*`), Edge `health-guardian` | PR #70 / DEC-2026-050 | **En production — périmètre limité à l'implantation du bouton ; `App.tsx`, `vite.config.ts` et `services/auth.ts` inchangés** |
