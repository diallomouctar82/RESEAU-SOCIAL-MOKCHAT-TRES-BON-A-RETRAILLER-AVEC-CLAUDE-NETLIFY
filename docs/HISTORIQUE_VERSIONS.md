...
| **v6.15.0** | 4 Septembre 2026 *(base seulement)* | **Santé Globale MokNet — 12 domaines, 52 lignes mesurées, 15 réparations à catalogue fermé, sauvegarde et action dans la même transaction ; lien profond (`#super-admin/sante`) ; le bandeau Super Admin dit enfin quelle version est à l'écran** | Super Admin, Supabase (`health_snapshots`, 13 fonctions `health_*`), Edge `health-guardian`, Navigation globale, Observabilité | PR #70 (brouillon) / DEC-2026-050 | **En essai — migration et fonction Edge déployées ; front NON en production ; en attente du constat écran de la Direction** |
