...
| **v6.43.1** | 5 Septembre 2026 | **Assistant IA Pré-Publication : le panneau reste toujours dans l'écran sur téléphone — champ du composeur à 16 px sur tactile (plus de zoom automatique iOS), voile qui suit la zone visible (zoom, clavier), carte 90 % / 96 % du voile, parties fixes jamais écrasées, en-tête et pied repliables, rien de retiré** | Réseau MOC (`components/AIPostAssistantModal.tsx`, blocs « ASSISTANT IA » et « COMPOSEUR A7 — CHAMP A 16 PX » d'`index.html`) | branche `claude/cleanup-home-interface-szp8qv` / DEC-2026-083 / PR brouillon | **Prête, non déployée — feu vert écrit attendu** : typage 0 erreur, 1645/1645 tests (109 fichiers), build OK, sonde sur neuf cas avant/après et parcours complet (`docs/captures/2026-09-05-assistant-ia-taille-telephone/`) |
...
### [Version 6.43.1] — 5 Septembre 2026 (Assistant IA Pré-Publication : le panneau reste toujours dans l'écran sur téléphone) — PRÊTE, NON DÉPLOYÉE

* **La demande** (Direction, 5 septembre) : « le panneau assistant /
  pré-publication s'ouvre trop grand et sort du cadre. L'utilisateur ne doit
  pas avoir à rétrécir manuellement. […] dimensionné correctement sur tous
  les téléphones, avec défilement interne si besoin, mais toujours dans
  l'écran, et les boutons importants doivent rester visibles et
  utilisables. »
* **Ce que le membre voit désormais sur téléphone** : le champ du composeur
  ne déclenche plus le zoom automatique d'iOS (16 px) ; le panneau tient dans
  la zone visible même si la page est zoomée ou le clavier ouvert ; seule la
  zone de contenu défile, l'en-tête, les onglets et les boutons « Annuler »
  et « Appliquer à ma publication » restent entiers et utilisables ; sur les
  plus petits écrans, l'en-tête se replie et « Annuler » passe au-dessus du
  bouton. Ordinateur inchangé.
...
* **Preuves** : typage 0 erreur, 1645/1645 tests (109 fichiers), build OK ;
  ...
  `docs/captures/2026-09-05-assistant-ia-taille-telephone/`. Production :
  non déployée ; feu vert écrit de la Direction attendu.
