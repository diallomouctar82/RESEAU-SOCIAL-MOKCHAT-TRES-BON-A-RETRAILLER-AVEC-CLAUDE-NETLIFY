...
| **v6.41.0** | 5 Septembre 2026 | **Accès public verrouillé : toute session relue depuis l'appareil est vérifiée par le serveur avant d'ouvrir l'interface — jeton refusé (périmé, révoqué, forgé, compte supprimé ou banni) → session effacée, écran de connexion ou de création de compte, sur tous les canaux (SMS, WhatsApp, Messenger, navigateurs mobile et ordinateur, navigateurs intégrés) ; session confirmée → Réseau MokNet comme avant ; serveur injoignable → tolérance dite ; domaine canonique déclaré (`www`, `http` → `https://moknet.net`, 301 forcées ; majuscules par la norme) ; rien supprimé** | Authentification (`services/auth.ts#verifierSession`, `App.tsx`), `netlify.toml` | branche `claude/moknet-public-access-auth-hu6yy7` / DEC-2026-079 | 🟡 **PRÊTE POUR PRODUCTION CONTRÔLÉE** — PR brouillon, Green Gate et revue indépendante en cours, feu vert écrit de la Direction attendu ; typage 0, 1 611 tests / 106 fichiers, build ✓, 70 mesures navigateur réel avant/après (7 canaux × 5 états) |
...
- `services/auth.ts` : `verifierSession()` — `GET /auth/v1/user` avec le jeton local ; verdict `valide` / `invalide` (session effacée par `signOut({ scope: 'local' })`) / `non-verifiee` (serveur injoignable, 5xx, 8 s sans réponse : session locale non expirée conservée, tolérance dite).
- `App.tsx` : sessions relues depuis le stockage (`getSession()`, `INITIAL_SESSION`) → verdict partagé, une seule vérification par jeton ; sessions émises par le serveur (`SIGNED_IN`, `TOKEN_REFRESHED`) → entrée comme avant. Aucun écran, aucun gestionnaire retiré.
...
- Tests : `tests/sessionValidee.test.ts` (9), `tests/appEntreeReseau.test.tsx` (+8), `tests/netlifyRedirectsCanoniques.test.ts` (6).
