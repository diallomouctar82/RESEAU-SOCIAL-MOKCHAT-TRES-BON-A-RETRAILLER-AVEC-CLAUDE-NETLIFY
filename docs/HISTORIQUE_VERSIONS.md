...
| **v6.15.0** | 4 Septembre 2026 | **La saturation d'un direct, de bout en bout : audit mesuré (SAT-0), plafond RÉELLEMENT posé à la création de la room d'après la machine réelle (SAT-1), porte de refus côté serveur (SAT-2), écran « Ce direct est complet » au lieu d'un « Connexion… » sans fin (SAT-3)** | Live / Directs, Fonction Edge `livekit-token`, Déploiement VPS LiveKit | PR #69 fusionnée → `8902cef` / DEC-2026-050 | **Courante (Active)** — code client en production sur `moknet.net` depuis le 4 septembre 2026 (bundle `index-DEDPIJvb.js`, étape 1 du plan d'activation). **Mais SAT reste INERTE** : la fonction Edge `livekit-token` est toujours en version 6, `prometheus_port` n'est pas posé sur le VPS (`/metrics` = 404), aucune room ne reçoit de plafond et la porte ne refuse personne. L'écran « Ce direct est complet » est dans le bundle et ne peut pas s'afficher. Étapes 2 à 5 en attente (`deploy/livekit/README.md` § SAT-1b) |
