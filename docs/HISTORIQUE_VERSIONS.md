...
| **v6.41.0** | 5 Septembre 2026 | **Accès public verrouillé : toute session, quel que soit l'événement qui l'apporte, est vérifiée par le serveur (une fois par jeton) avant d'ouvrir l'interface — jeton refusé (périmé, révoqué, forgé, compte supprimé ou banni) → session effacée, écran de connexion ou de création de compte, sur tous les canaux (SMS, WhatsApp, Messenger, navigateurs mobile et ordinateur, navigateurs intégrés) ; session confirmée → Réseau MokNet comme avant ; serveur injoignable → tolérance dite ; domaine canonique déclaré (`www`, `http` → `https://moknet.net`, 301 forcées ; majuscules par la norme) ; rien supprimé** | Authentification (`services/auth.ts#verifierSession`, `App.tsx`), `netlify.toml` | branche `claude/moknet-public-access-auth-hu6yy7` / DEC-2026-079 | 🟡 **PRÊTE POUR PRODUCTION CONTRÔLÉE** — PR brouillon, Green Gate et revue indépendante en cours, feu vert écrit de la Direction attendu ; typage 0, 1 612 tests / 106 fichiers, build ✓, 70 mesures navigateur réel avant/après (7 canaux × 5 états) |
...
### [Version 6.41.0] — 5 Septembre 2026 (Accès public : verrou d'entrée vérifié par le serveur, domaine canonique)
**Référence** : DEC-2026-079 · branche `claude/moknet-public-access-auth-hu6yy7` · statut 🟡 prête pour production contrôlée (feu vert écrit attendu).
