...
| **v6.24.0** | 5 Septembre 2026 | **SAT-6 — le bouton de secours du direct, réservé à l'Admin Général : relancer la room d'un direct ou le clore depuis la Santé Globale, sans SSH — rang relu en base avant toute lecture et de nouveau au geste, diagnostic qui compte les présents réels, confirmation signée cinq minutes, re-mesure avant verdict, journal `audit_logs` ; deux défauts pré-existants corrigés en chemin (modales de la Santé Globale cadrées sur l'onglet au lieu de la fenêtre, spectateur éjecté à son arrivée par une course du roster)** | Santé Globale (Super-Admin), Edge `health-guardian` v4, Live / Directs | branche `claude/lives-directs-sat6`, PR #88 / DEC-2026-059 | **Remplacée par la v6.25.0 le 5/09/2026 à 11:14 UTC (son client SAT-6 est servi dans le même bundle `index-JAHL7yTa.js`) — était en cours de mise en production contrôlée (validation de la Direction le 5/09/2026 à 10h40 UTC)** : fonction Edge v4 **déployée** à 10:49:57 UTC (artefact régénéré depuis `ae0d5eb`, 66 588 octets, source relue identique octet pour octet ; retour arrière v3 régénéré et vérifié identique à l'ancienne production avant le déploiement), backend vérifié à 10:53 UTC avec trois vraies sessions (`probe` 45 lignes comme la v3, 403 pour `user`, lecture seule pour `admin`, gestes pour `super_admin`), geste réel « Clore » joué à 10:55 UTC sur un direct de banc privé (journal `audit_logs` réel) ; client : PR #88 fusionnée en squash sur `main` — bundle servi et contrôle du tableau de bord réel consignés par la présente PR de mémoire vivante — le client SAT-6 est porté depuis `4486cc4` dans le bundle courant de `moknet.net` (empreintes SAT-6 présentes dans le bundle servi à ce jour, ancien `index-BKkSLQXf.js` de la fusion → 404), panneau « Secours du direct » prouvé visible et actif dans le tableau de bord réel (banc navigateur 11 OK / 0 DÉFAUT sur la source servie par la production : gestes « Relancer »/« Clore » actifs pour un `super_admin`, « Lecture seule » sans bouton pour un `admin`, panneau absent pour un `user`) ; greffée sur la Santé Globale v2 (v6.22.0). Banc réel contre un LiveKit vivant, trois vrais comptes et la base réelle : **51 OK / 0 DÉFAUT** (passe 4, après trois passes correctives 44/45, 38/48, 49/50 ; **passe 5 rejouée sur l'arbre fusionné : 51 OK / 0 DÉFAUT**) ; tsc 0 · vitest 1128/1128 · build |
| **v6.25.0** | 5 Septembre 2026 | **Assistant IA de Santé Globale — vocal et texte, avatar existant (Directeur Diallo), Analyser avec progression réelle et bilan dit, Réparer tout le lot / les rouges seuls / les oranges seuls / un domaine / un point, une seule confirmation par lot, pourcentage à chaque boucle, échec → cause + étapes exactes + lien, arrêt immédiat si incontrôlable et retour à l'état stable (restauration du lot), « Diagnostiquer » quand le rang ne permet pas d'appliquer** | Super-Admin / Santé Globale, `services/health/assistant/*`, moteur vocal partagé, passerelle IA | branche `claude/moknet-security-audit-ohfwc1` / DEC-2026-060 / PR #91 | **Remplacée par la v6.26.0 le 5/09/2026 à 12:38 UTC — était en production contrôlée depuis le 5/09/2026 à 11:14 UTC** : PR #91 fusionnée en squash → `main` `b36318b` (Green Gate run 33962624632 vert sur `5b265dd`), `moknet.net` sert `index-JAHL7yTa.js` avec les empreintes de l'Assistant (ancien bundle `index-BKkSLQXf.js` → 404, `cache-control: max-age=0, must-revalidate`) ; typage 0 erreur, 1177/1177 tests (83 fichiers, 39 nouveaux) sur la branche reconstruite depuis `main` après SAT-6, build OK, harnais sur mesures de production du 5/09 (rang réel admin + banc super_admin simulé, ordinateur et téléphone, 0 erreur JS, `docs/captures/2026-09-05-sante-assistant/`) |
| **v6.26.0** | 5 Septembre 2026 | **Composeur « A7, rail latéral » du Réseau MOC (choix de la Direction parmi dix variantes de la série A) et studio « Visuel IA » B10 intégré à la publication (parmi dix variantes de la série B) : quatre médias en orbes de cristal en rail à gauche du champ (sous le champ sur téléphone), Assistant IA en orbe d'or dans le champ, quatre actions IA nommées (améliorer le style, traduire, hashtags → modale existante sur le bon onglet ; Visuel IA → studio intégré), compteur de caractères, Brouillon \| Publier groupés, rien de supprimé, avatar intouché ; studio plein écran sombre : prompt (l'IA renvoie des réglages) + réglages manuels, visage & cheveux, lumière, cinéma, texte, cadrage, vidéo, vrai pipeline de pixels, insertion d'un JPEG à pleine résolution** | Réseau MOC (composeur), `VisuelIAStudio`, `services/visuelIA.ts`, `AIPostAssistantModal` (prop `initialTool`), index.html | branche `claude/cleanup-home-interface-szp8qv` / DEC-2026-061 / PR #93 | **Remplacée par la v6.37.0 le 5/09/2026 à 13:01 UTC — était en production contrôlée depuis le 5/09/2026 à 12:38 UTC** : PR #93 fusionnée en squash → `main` `36e0a44` (Green Gate vert sur `94514df`, run 33966471120, et sur `main`, run 33966591144), revue indépendante passée (1 bloquant, 4 importants et 10 mineurs corrigés, régression relevée en contre-vérification corrigée, verdict final « PRÊT »), `moknet.net` sert `index-ggiwFepQ.js` depuis 12:38:39 UTC (blocs « COMPOSEUR A7 » et « VISUEL IA » présents, marqueurs du bundle présents, ancienne barre absente, ancien bundle `index-JAHL7yTa.js` → 404), miroir local vérifié dans Chromium ; typage 0 erreur, 1233/1233 tests (86 fichiers, 57 dédiés), build OK, captures avant/après (`docs/captures/2026-09-05-composeur-a7-studio-b10/`) |

| **v6.27.0** | 4 Septembre 2026 | **Studio Avatar : avatar par défaut défini par l'Admin-Général pour tous les nouveaux comptes ; avatar personnel des membres Pro (accès → photo → consentement → nom → génération → aperçu), révocable ; console admin protégée contre un `localStorage` antérieur au réglage** | Studio & Création, Espace Super-Admin, Profil & identité | DEC-2026-062 | **Développée & testée — NON déployée (autorisation de production requise)** |
| **v6.28.0** | 4 Septembre 2026 | **Avatar vivant de l'Architecte : visage à la place du bouton, 8 états animés (dont mode allégé et hors ligne), synchro labiale sur la voix réellement prononcée ; Super-Admin — changer l'avatar, remettre par défaut, animations, voix** | Diallo OS & Architecte, Espace Super-Admin, Moteur vocal | AI Core playbook 15 / DEC-2026-063 | **Développée & testée — NON déployée (validation visuelle Direction requise)** |
| **v6.29.0** | 4 Septembre 2026 | **Portrait vivant de l'Architecte : vraie photo qui respire, cligne (paupières de peau), parle (lèvre du haut fixe, cavité et dents), bouge la tête ; calage relevé sur grille ; page publique `/architecte` qui parle à voix haute avec la voix du navigateur** | Diallo OS & Architecte, Espace Super-Admin | AI Core playbook 15 / DEC-2026-064 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation visuelle Direction requise)** |
| **v6.30.0** | 4 Septembre 2026 | **Portrait vivant fluide et naturel : rendu Canvas, fond fixe, mâchoire déformée en continu (cou immobile), paupières par œil, regard qui bouge, hochements sur le phrasé, lèvres entrouvertes en parlant ; banc à pose fixée** | Diallo OS & Architecte, Espace Super-Admin | AI Core playbook 15 / DEC-2026-065 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation visuelle Direction requise)** |
| **v6.31.0** | 5 Septembre 2026 | **Synchro bouche-voix réelle, mesurée avec le son : amplitude efficace temporelle normalisée par adaptation (fin de la bouche ouverte sur le souffle), avance de 60 ms (décalage mesuré 0 ms, corrélation 0,80 contre 0,15), lissages en temps valables à toute cadence ; avatar branché tel quel sur la chaîne vocale du Super-Admin (relais automatique entre fournisseurs, dernier relais voix du navigateur, aucune clé exposée) ; test avec son livré sur `/architecte`** | Diallo OS & Architecte, Espace IA (chaîne vocale) | AI Core playbook 15 / DEC-2026-066 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation Direction requise)** |
| **v6.32.0** | 5 Septembre 2026 | **Visèmes acoustiques et gestes portés par la parole : la forme de la bouche vient du spectre de la voix (« a » ouvert, « i » étiré, « ou » arrondi, « m »/« p » jointes, dents sur « s »), ouverture de parole ramenée à 60 % de la course ; hochements sur les temps forts, sourcils et relèvement en début de phrase, clignement et regard dans les pauses, inclinaison en fin de phrase — tous par ressorts, déclenchés par la voix ; avance temps réel 90 ms avec latence d'appareil déduite ; film image par image VP9/Opus vérifié indépendamment (corrélation 0,81 à 0 ms)** | Diallo OS & Architecte, Espace IA (chaîne vocale) | AI Core playbook 15 / DEC-2026-067 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation Direction requise)** |
| **v6.33.0** | 5 Septembre 2026 | **Harmonisation voix / lèvres / gestes / yeux d'après les compétences AI Core (playbook 15 § 3-5-7, playbook 09) : les lèvres précèdent le son de 60 ms (coarticulation sans retard de phase, retard d'affichage mesuré 80 ms déduit, retard du son 200 ms) ; pendant la parole, les mouvements sans lien avec la voix s'effacent et les gestes déclenchés par la voix deviennent visibles ; clignements aux pauses de la voix seulement ; vidéo avant/après côte à côte sur la même phrase, contrôle indépendant dans les pixels (−67 ms, 0,78) et en temps réel (−69 ms)** | Diallo OS & Architecte, Espace IA (chaîne vocale) | AI Core playbooks 15, 09, 06 / DEC-2026-068 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation Direction requise)** |
| **v6.34.0** | 5 Septembre 2026 | **Visèmes phonétiques alignés sur le son (playbook 15 § 5 au sens propre) : le texte est transcrit en phonèmes par règles et calé sur le clip HD avant la lecture (alignement forcé, Viterbi semi-markovien, 67–114 ms pour 8 s) — lèvres jointes sur b/p/m, dents sur f/v/s, voyelles ouvertes, étirées ou arrondies ; gestes, regard et clignements PLANIFIÉS sur la même partition (hochement 90 ms avant la syllabe accentuée, sourcils avant le premier mot, clignement dans les pauses) ; couplage yeux-tête ; défaut corrigé : le clignement de parole n'était jamais joué dans le navigateur (deux horloges) ; contrôle indépendant dans les pixels (pauses 3/3, voyelles 9/9, fermetures toutes sous 0,135) ; vidéo avant/après même phrase** | Diallo OS & Architecte, Espace IA (chaîne vocale) | AI Core playbooks 15, 09 / DEC-2026-069 | **Développée & testée — NON déployée en production (prévisualisation de branche, validation Direction requise)** |
...
| **v6.43.0** | 5 Septembre 2026 | **Studio Live : bouton « Quitter le direct » clair et sortie propre selon le rôle, sans jamais recharger la page — un nouveau `handleQuitLive` distingue l'animateur (compte-rendu de fin conservé, `handleEndLive` inchangé) du spectateur (média coupé, session quittée, vue refermée par un changement d'état côté `App.tsx`) ; libellé texte VISIBLE « Quitter » sur téléphone et ordinateur, cible tactile 44 px, hors de la zone défilante** | Réseau MOC / Studio Live (`components/SocialLive.tsx`), garde-fou CI (`tests/liveQuitButton.test.tsx`) | branche `claude/live-lv2-son-direct-prive`, PR #112 / DEC-2026-082 | **Courante (Active) — EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 17:23 UTC** : PR #112 fusionnée en squash sur `expectedHeadSha` `97abca9` → `main` `f6948b0` (Green Gate vert sur `97abca9` puis sur `main` run #253 `33980817639` `success`) ; `main` a depuis avancé (PR #114 gouvernance, PR #111 v6.42.1 Architecte), le bundle servi par `moknet.net` est désormais `index-C8Y6seFM.js` et porte toujours les marqueurs `live-quit-button` et « Quitter le direct » (ancien libellé « Quitter ou terminer le Live » absent, bundle de la fusion `index-C_IAplN2.js` → 404) ; typage 0, 1635/1635 tests (108 fichiers, dont `liveQuitButton` 3/3 et la garde `livekitClientPin` 5/5), build propre, banc navigateur réel avant/après 12/12 (ordinateur 1440×900 + téléphone 390×844) ; aucune migration, aucune fonction Edge, aucune donnée touchée |
| **v6.42.0** | 5 Septembre 2026 | **Accès public verrouillé : toute session, quel que soit l'événement qui l'apporte, est vérifiée par le serveur (une fois par jeton) avant d'ouvrir l'interface — jeton refusé (périmé, révoqué, forgé, compte supprimé ou banni) → session effacée, écran de connexion ou de création de compte, sur tous les canaux (SMS, WhatsApp, Messenger, navigateurs mobile et ordinateur, navigateurs intégrés) ; session confirmée → Réseau MokNet comme avant ; serveur injoignable → tolérance dite ; domaine canonique déclaré (`www`, `http` → `https://moknet.net`, 301 forcées ; majuscules par la norme) ; rien supprimé** | Authentification (`services/auth.ts#verifierSession`, `App.tsx`), `netlify.toml` | branche `claude/moknet-public-access-auth-hu6yy7` / DEC-2026-081 | **Remplacée par la v6.43.0 le 5/09/2026 à 17:23 UTC — était EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 17:01 UTC** : PR #105 fusionnée en squash → `main` `daa6575` le 5/09/2026 à 17:00 UTC (Green Gate vert sur la tête exacte `4247523`, run 33979505971 ; sur `main`, run 33979639128) après contrôle de coordination (`main` = `6f6f03d` revérifié dans la minute, aucune autre fusion en cours) ; `moknet.net` sert `index-CacRDIgE.js` depuis 17:01:54 UTC (5 432 686 octets) avec les quatre marqueurs du verrou (« refus du serveur d'authentification (», « serveur d'authentification sans verdict », « Session locale refusée par le serveur », « le jeton ne correspond pas à l'utilisateur de la session locale »), ancienne formulation absente, ancien bundle `index-6ZrCib2c.js` → 404, `cache-control: public,max-age=0,must-revalidate` ; dix écritures du domaine tracées après fusion (`www`, `http`, majuscules) → toutes sur `https://moknet.net/` ; fumée Chromium sur le bundle réellement servi (miroir local) : vierge → connexion, session refusée → connexion, session confirmée → interface, rejeu `SIGNED_IN` → connexion maintenue, ordinateur et téléphone, 8/8 ; typage 0, 1 632 tests / 107 fichiers, build ✓, 70 mesures avant/après + rejeu ; revue indépendante « À CORRIGER » puis contre-vérifications « PRÊT » (×4) |
| **v6.39.0** | 5 Septembre 2026 | **Avatar vivant depuis une photo : option Super-Admin « Créer ou remplacer l'avatar vivant depuis une photo » — analyse automatique dans le navigateur (MediaPipe : 478 repères du visage, silhouette), cadrage au portrait d'usine, rig et bouche calculés, masque de détourage propre à la photo, aperçu vivant actuel / nouveau, validation, sauvegarde, retour arrière ; le modèle vidéo validé ne se joue que sur son portrait d'origine ; aucun panneau par-dessus l'application ; rien supprimé** | Super-Admin (carte `AdminArchitecteAvatarCard`), `services/architecte/photoAvatar.ts`, `photoAvatarEngine.ts`, `architecteAvatar.ts`, `sequences.ts`, barre flottante, `public/architecte/modeles/`, banc `design-lab/banc/super-admin-avatar.html` | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-077 | **Remplacée par la v6.40.0 le 5/09/2026 à 15:13 UTC — était en production contrôlée depuis le 5/09/2026 à 14:25 UTC** : PR #101 fusionnée en squash → `main` `f64f7ee` (Green Gate vert sur `5bb9566`, run 33971591543), `moknet.net` sert `index-Bm6woHcd.js` avec les marqueurs de l'option, morceau MediaPipe et modèles servis (tailles exactes), ancien bundle `index-D0ScDC87.js` → 404 ; typage 0, 1 588 tests / 104 fichiers, build ✓, essai navigateur réel Super-Admin + application, ordinateur + téléphone (14 captures, mesures) |
| **v6.40.0** | 5 Septembre 2026 | **Réseau MOC : même disposition des boutons de publication et des options sur ordinateur et téléphone — sur téléphone, le rail des médias (Photo, Vidéo, Document, Voix) garde ses libellés et descend sous l'avatar dans la même colonne, les quatre actions IA restent nommées sur une ligne sous leur intitulé, et la bande « Aurore » passe du rail défilant à une grille 4 × 4 (damier conservé) sans aucun défilement horizontal ; ordinateur inchangé ; repli conservé pour les cartes très étroites ; CSS seulement, avatar intouché** | Réseau MOC (composeur A7, bande Aurore), index.html | branche `claude/cleanup-home-interface-szp8qv` / DEC-2026-078 (v6.38.0 et v6.39.0, DEC-2026-075 à 077, prises entre-temps par les PR #98 et #101 du Studio Avatar) | **Remplacée par la v6.40.1 le 5/09/2026 à 15:27 UTC (son contenu est servi dans le même bundle `index-CMCLckXy.js`) — était courante (Active) — EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 15:13 UTC** : PR #100 fusionnée en squash → `main` `a615593` à 15:12:30 UTC sur feu vert écrit (Green Gate vert sur `a921a2f`, runs 33973191332 et 33973186563, et sur `main`, run 33974090869), `moknet.net` sert la nouvelle feuille depuis 15:13:19 UTC (bundle `index-Bm6woHcd.js` inchangé — CSS seulement ; marqueurs présents, aucun seuil 300/360 px, aucune règle de défilement dans la bande, ancien bundle `index-DnzvQMzK.js` → 404), miroir local vérifié dans Chromium, revue indépendante « À CORRIGER » (6 constats corrigés) puis contre-vérification « PRÊT » ; typage 0 erreur, 1589/1589 tests (104 fichiers), build OK, captures et mesures avant/après sur six écrans (`docs/captures/2026-09-05-reseau-meme-disposition-telephone/`, script `mesurer.cjs` versionné) |
| **v6.40.1** | 5 Septembre 2026 | **Assistant IA de Santé Globale : le champ de dialogue est placé juste sous la conversation (touche Entrée ou bouton Envoyer), placeholder « Écrivez ici : … » ; rien à activer, le dialogue est ouvert à tout administrateur, seul l'acte d'appliquer une réparation exige le rang Admin Général** | Super-Admin / Santé Globale (`components/admin/HealthAssistant.tsx`) | branche `claude/moknet-security-audit-ohfwc1` / DEC-2026-060 (ajustement) / PR #104 | **Remplacée par la v6.41.0 le 5/09/2026 à 15:51 UTC — était en production contrôlée depuis le 5/09/2026 à 15:27 UTC** : PR #104 fusionnée en squash → `main` `d3415b4` (Green Gate run 33974730535 vert sur `eb8fea5`), `moknet.net` sert `index-CMCLckXy.js` (placeholder « Écrivez ici : » présent dans le bundle, ancien bundle `index-Bm6woHcd.js` → 404) ; typage 0 erreur, 1588/1588 tests (104 fichiers), harnais Chromium sur mesures de production du 5/09 (quatre parcours, 0 erreur JS, captures régénérées dans `docs/captures/2026-09-05-sante-assistant/`) |
| **v6.38.0** | 5 Septembre 2026 | **Zéro obstruction strict : l'Architecte guide dans l'interface visible, jamais par-dessus — le panneau de conversation ne s'ouvre plus JAMAIS de lui-même (réponse longue, image, document ou vidéo : signalés par un point sur la flèche, dépliés seulement par la personne) ; Écrire ouvre une seule ligne de saisie ; rouvrir repart toujours de la barre minimale ; panneau borné à min(36 vh, 18 rem) ; obstruction mesurée après une réponse longue : 2,6 % de l'écran sur ordinateur, 6,8 % sur téléphone, panneau absent, dock libre ; rien supprimé ; **identité officielle « Je suis l'Architecte de Vision Smart » (jamais « avatar » ni « IA »), description validée servie sans modèle, ton bref, dit quand il ne comprend pas ; entrée directe sur Réseau MokNet dès connexion ou inscription, sans rechargement** | Avatar de l'Architecte (barre flottante `ArchitecteFloatingBar`, cerveau `architecteBrain`), entrée `App.tsx`, banc `design-lab/banc/sculpture.html` | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-075, DEC-2026-076 | **Remplacée par la v6.39.0 le 5/09/2026 à 14:25 UTC — était en production contrôlée depuis le 5/09/2026 à 14:11 UTC** : PR #98 fusionnée en squash → `main` `022a224` (Green Gate vert sur `4774573`, run 33970158515), `moknet.net` sert `index-D0ScDC87.js` avec les huit marqueurs attendus (ancien bundle `index-DnzvQMzK.js` → 404) ; typage 0, 1 573 tests / 102 fichiers, essai navigateur réel ordinateur + téléphone (16 captures, obstruction mesurée), test d'entrée App réelle |
| **v6.37.0** | 5 Septembre 2026 | **La sculpture vivante : l'avatar validé (vidéo HeyGen) remplace directement le bouton flottant de l'Architecte, au même emplacement, détouré image par image (204 mattes relevées en local, vidéo empilée couleurs + alpha recomposée dans un canevas), sans cadre ni page ; seul visible par défaut ; au clic il parle (modèle validé, dans le geste) et n'ouvre qu'une petite barre d'icônes à côté de lui — accueil et micro attendent la fin de la vidéo ; panneau étroit à la demande (flèche) ; **zéro obstruction mesurée** (0,9 % de l'écran au repos, 2,4 % barre ouverte sur ordinateur ; dock libre sur téléphone) ; calage vidéo ↔ portrait mesuré sur les pupilles ; lecteur corrigé d'après la relecture indépendante (double appui, fin réelle, arrêt par cadre) ; rien supprimé, rig 2D en repli** | Avatar de l'Architecte (`ArchitecteAvatar` variante `sculpture`, `ArchitecteSequenceCutout`, barre flottante), `public/architecte/vision-smart-heygen.cutout.*`, `architecte-silhouette.png`, banc `design-lab/banc/sculpture.html` | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-073 / PR #71 | **Remplacée par la v6.38.0 le 5/09/2026 à 14:11 UTC — était en production contrôlée depuis le 5/09/2026 à 13:01 UTC** : PR #71 fusionnée en squash → `main` `d9c2c2c` (Green Gate vert sur `9dd10f1`, run 33967521429), `moknet.net` sert `index-DnzvQMzK.js` avec les marqueurs de la sculpture (ancien bundle `index-ggiwFepQ.js` → 404), actifs détourés servis avec tailles et empreintes attendues, page `/architecte` vérifiée en Chromium réel ; typage 0, 1 562 tests / 101 fichiers, build ✓, essai navigateur réel ordinateur + téléphone (captures, obstruction mesurée) |
| **v6.41.0** | 5 Septembre 2026 | **La photo validée devient l'avatar vivant de l'Architecte : portrait, masque et calage d'usine relevés par le moteur de production ; voix unique (la voix attitrée de l'Architecte sur la phrase officielle « je suis l'Architecte de Vision Smart ») ; modèle vidéo HeyGen généré depuis la photo validée, détouré image par image ; onglet Super-Admin dédié « Avatar de l'Architecte » à enregistrement immédiat ; réglage partagé de la plateforme (`platform_settings`, migration non appliquée) ; la sculpture se retire en pastille pendant la saisie (« Publier » atteignable sur téléphone)** | Diallo OS & Architecte (module 01), Super-Admin (tableau de bord), réglages partagés, `public/architecte/`, bancs `design-lab/banc/super-admin.html` et `reperes.html` | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-079 | **Remplacée par la v6.41.1 (5/09/2026 à 16:47 UTC) — était EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 15:51 UTC** : PR #107 fusionnée en squash → `main` `86b521b` à 15:50 UTC (Green Gate vert sur `f586342`, run 33975758223 ; `main` vérifié inchangé `ba1ba7d` à 15:49 UTC ; Green Gate sur `main` run 33975984731), `moknet.net` sert `index-Bvy0oNZ6.js` (marqueurs de l'onglet, du réglage partagé, de la phrase officielle et du retrait ; portrait 20 380 o, masque 17 507 o, voix 1 602 170 o, vidéos 1 558 047 / 445 523 / 1 810 971 / 599 002 o, poster 5 778 o, légendes 274 o servis ; ancien bundle `index-CMCLckXy.js` → 404) ; typage 0 erreur, 1595/1595 tests (104 fichiers), build ✓, essais navigateur réel Super-Admin (ordinateur + téléphone), application (sculpture + vidéo du portrait validé, obstruction 0,9 / 2,4 % et 2,1 / 6,8 %), fil réel (« Publier » : 2 301 px² couverts au repos → 0 px² pendant la saisie) ; 4 crédits HeyGen + 8 crédits Arcads ; migration `platform_settings` non appliquée (feu vert requis) |
| **v6.41.1** | 5 Septembre 2026 | **Assistant IA Pré-Publication : sur téléphone, la modale passe au-dessus de la barre du bas — « Appliquer à ma publication » visible et cliquable, parcours complet jusqu'à la publication ; portail sur `<body>`, racine inerte, dialogue accessible, rien de retiré** | Réseau MOC (`components/AIPostAssistantModal.tsx`, bloc « ASSISTANT IA » d'`index.html`) | branche `claude/cleanup-home-interface-szp8qv` / DEC-2026-080 / PR #109 | **Remplacée par la v6.42.0 le 5/09/2026 à 17:01 UTC — était en production contrôlée depuis le 5/09/2026 à 16:47 UTC** : PR #109 fusionnée en squash sur la tête exacte `e5ed7eb` → `main` `81c66c8` à 16:46:29 UTC sur feu vert écrit (Green Gate vert sur `e5ed7eb` run 33978405335 et sur `main` run 33978900871), `moknet.net` sert `index-6ZrCib2c.js` depuis 16:47:11 UTC (sept marqueurs présents, ancien bundle `index-Bvy0oNZ6.js` → 404) ; typage 0 erreur, 1602/1602 tests (105 fichiers), build OK, parcours avant/après sur 390 × 844, 360 × 800 et 1440 × 900 (`docs/captures/2026-09-05-assistant-ia-modale-dock/`) |
| **v6.42.1** | 5 Septembre 2026 | **Architecte : plus de bande sombre au sommet du cadre rond de `/architecte` — le moteur de cadrage comble le débord d'une photo trop serrée par un prolongement adouci du fond (et le signale à l'Admin-Général) ; portrait d'usine, affiche et vidéo HeyGen régénérés sans nouveau crédit** | Diallo OS & Architecte (module 01) : moteur photo (`photoAvatarEngine.ts`, `photoAvatar.ts`), `public/architecte/`, registre des séquences ; Super-Admin (avertissement de l'option « depuis une photo ») | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-079 (ajustement) | **En production contrôlée depuis le 5/09/2026 à 17:48 UTC** (version Courante par numérotation : v6.43.0, en production depuis 17:23 UTC — les deux sont portées par le bundle servi) : PR #111 fusionnée en squash → `main` `f6d9a16` à 17:47 UTC (Green Gate vert sur la tête exacte `d5550ed`, run 33981843750 ; `main` vérifié inchangé `e9595e5` à 17:46:58 UTC ; revue indépendante « À CORRIGER » puis contre-vérification « PRÊT » sur `bcb9d75` et sur `d5550ed` ; Green Gate sur `main` run 33982039577), `moknet.net` sert `index-C8Y6seFM.js` depuis 17:48 UTC (marqueurs du remède présents, nouvelles empreintes présentes, anciennes absentes ; portrait 21 368 o, silhouette 16 953 o, vidéos 1 391 269 / 561 306 o, affiche 6 122 o, vidéos détourées et voix inchangées, tous servis aux tailles exactes ; ancien bundle `index-C_IAplN2.js` → 404 ; page `/architecte` 200) ; fumée Chromium sur la page servie à 17:48 UTC : luminance de la bande 132 pour le portrait et 132 pour la vidéo, sur ordinateur et téléphone, 0 erreur ; typage 0 erreur, 1642/1642 tests (109 fichiers) sur `d5550ed`, build ✓ ; captures avant/après sur trois écrans (`docs/captures/2026-09-05-architecte-bande-sombre/`) |
| **v6.45.0** | 5 Septembre 2026 | **Super-Admin : onglet « Versions stables & restauration contrôlée » — les dernières versions livrées avec nom, date, commit, PR, module, statut de production et preuves ; ce que `moknet.net` sert réellement (lecture sans cache de `/version.json`, carte d'identité écrite au build, puis du bundle) ; ordre de restauration contrôlée, jamais à l'aveugle : pré-contrôles un à un, saisie exacte de la version, motif, reconnaissance des oranges, ordre nominatif journalisé (réglage partagé `restauration_versions` + journal d'audit), procédure exacte (republication du déploiement Netlify ou revert Git), vérification de la version servie après coup** | Super-Admin — Sécurité & infrastructure (`components/admin/AdminStableVersionsTab.tsx`, `services/versions/`, `vite-plugins/versionJson.ts`, `vite.config.ts`, `components/AdminDashboard.tsx`, note de renvoi dans `AdminWorkflowsAndBackupTab.tsx`) | Vision Smart AI Core / DEC-2026-086 | 🟡 **En préparation** — PR brouillon, production contrôlée sur feu vert écrit |
...
### [Version 6.45.0] — 5 Septembre 2026 (Super-Admin : versions stables et restauration contrôlée) — EN PRÉPARATION
**Référence** : DEC-2026-086 · branche `claude/stable-versions-restore-system-bobj0l` · statut 🟡 en préparation (PR brouillon ; production contrôlée seulement sur feu vert écrit de la Direction, une fois Green Gate vert, revue indépendante « PRÊT » et preuves versionnées).

**Ce qui existait** : l'onglet « Workflows & Sauvegarde » porte depuis la v6.3 un sous-onglet « Versions » aux données figées (v6.3.0 « courante », v6.2.0, v6.1.0, sommes de contrôle inventées) dont la « restauration intelligente » ne rejoue que la configuration locale de la console (modules, connecteurs, modèles, réglages) — il n'a jamais décrit le code livré, et il annonçait v6.3.0 comme courante alors que `moknet.net` servait la v6.42.1. Savoir quelle version la production sert exigeait de comparer le nom du bundle (`index-XXXX.js`) à la mémoire vivante, et rien dans la console ne permettait de préparer un retour à une version saine.

**Ce qui a été demandé** (Direction, 05/09) : « créer dans le tableau de bord Administrateur Général ou Super Admin un système pour voir les versions stables et restaurer une version saine en cas de régression […] afficher au moins les trois dernières versions stables avec nom, date, commit, PR, module, statut de prod et preuves. Bouton de restauration contrôlée, jamais à l'aveugle. […] Ne mélange rien hors de ce périmètre. »

**Ce qui a été modifié** :
- `services/versions/versionDuCode.ts` : la version que ce code déclare (`VERSION_DU_CODE`), source unique, avancée par chaque mission qui livre du code.
- `services/versions/stableVersions.ts` (pur) : registre typé de huit versions (v6.40.0 → v6.43.0 fusionnées, relevées dans cette mémoire vivante — commit complet, PR, heure de fusion UTC, bundle à la fusion, preuves, risques ; plus la version en préparation) ; identification de la version réellement servie à partir de `/version.json` (commit consigné → identifiée ; version du registre mais commit non consigné → « déclarée », dit en toutes lettres ; version absente → « inconnue du registre ») puis, sans `version.json`, du bundle servi ; sept pré-contrôles (cible consignée, cible éprouvée, production lisible, cible différente de la servie, schéma de base, données réelles, configuration servie) — un rouge refuse, un orange doit être reconnu ; procédure exacte (voie A : republier le déploiement Netlify du commit cible, aperçu d'abord, « Publish deploy », verrou de publication automatique dit ; voie B : `git revert` des squashs postérieurs, un par version, puis typage, suite, build, PR, Green Gate) ; verdict après restauration (vert seulement si la cible est servie, orange si la version déclarée porte un autre commit, rouge sinon, blanc si non identifiable).
- `services/versions/restaurationControlee.ts` : lecture de la production sans cache (`/version.json?v=…` puis `/?v=…` — clé d'horodatage parce que le service worker sert « cache d'abord » toute requête GET de même origine hors navigation et ignore `cache: 'no-store'`), carte d'identité de l'onglet courant (`__MOKNET_BUILD__`), journal des ordres (réglage partagé `platform_settings` clé `restauration_versions`, copie locale, ligne `security` dans le journal d'audit de la console). Aucun secret Netlify ou GitHub dans le navigateur : ce module prépare, trace et vérifie ; la publication reste manuelle et nominative.
- `vite-plugins/versionJson.ts` + `vite.config.ts` : `version.json` émis à la racine du build (version déclarée, commit `COMMIT_REF` ou `git rev-parse HEAD`, `DEPLOY_ID`, branche, contexte, heure, bundle d'entrée) et `__MOKNET_BUILD__` injecté dans le code.
- `components/admin/AdminStableVersionsTab.tsx` : l'onglet — production servie (verdict, commit, déploiement, bundle, source ; « Relire la production » ; « votre onglet exécute … » ; alerte si une version plus récente est en ligne), cartes des versions (statut relatif à la version servie : servie / remplacée / fusionnée après / en préparation ; le bouton « Restaurer cette version… » est désactivé pour la version servie et pour une version non fusionnée), panneau d'ordre (pré-contrôles avec verdict, formulaire verrouillé tant que la version exacte n'est pas saisie et qu'un motif de dix caractères n'est pas donné, case de reconnaissance des oranges, un seul ordre par panneau), ordre enregistré (identifiant `RST-AAAAMMJJ-HHMM-xxxx`, auteur, heure UTC, journal partagé ou copie locale dit), procédure A et B (lien vers les déploiements Netlify, commandes copiables), « Vérifier la version servie maintenant » (verdict consigné dans l'ordre), journal des ordres.
- `components/AdminDashboard.tsx` : onglet « Versions stables » (`data-testid="admin-onglet-versions"`) après « Santé Globale » ; `components/admin/AdminWorkflowsAndBackupTab.tsx` : note de portée dans l'ancien sous-onglet « Versions » (configuration locale) avec bouton vers le nouvel onglet — rien retiré.
- Tests : `tests/stableVersions.test.ts` (25 : registre ↔ mémoire vivante — ligne, section, commit court, PR, décision de chaque version —, ordre de fusion, identification, pré-contrôles sur cas réels, procédure, verdict), `tests/versionJsonBuild.test.ts` (4), `tests/adminStableVersionsTab.test.tsx` (11 : parcours complet, refus injoignable, orange à reconnaître, vérification rouge puis verte, journal indisponible dit), `tests/adminDashboardOngletVersions.test.ts` (4 : garde-fou de structure, rien ne disparaît).

**Preuves** : consignées dans `docs/captures/2026-09-05-versions-stables-restauration/` (README, mesures JSON, captures ordinateur 1440×900, tablette 820×1180, téléphone 390×844, avant sur `origin/main` et après) ; typage, suite complète, build, Green Gate et revue indépendante consignés dans la PR puis dans DEC-2026-086 à la production.

### [Version 6.43.0] — 5 Septembre 2026 (Studio Live : bouton « Quitter le direct » clair, sortie propre selon le rôle)
**Référence** : DEC-2026-082 · mission LV-2 point 1 · branche `claude/live-lv2-son-direct-prive` · statut 🟢 en production contrôlée depuis le 5/09/2026 à 17:23 UTC (PR #112 → `main` `f6948b0`). `main` a depuis avancé (PR #114 gouvernance, PR #111 v6.42.1 Architecte) : `moknet.net` sert désormais `index-C8Y6seFM.js`, qui porte toujours les marqueurs `live-quit-button` et « Quitter le direct » (bundle de la fusion `index-C_IAplN2.js` → 404).

**Ce qui existait** : le bouton de sortie du Studio Live était une icône rouge seule (`PhoneOff`, `aria-label="Quitter ou terminer le Live"`, `onClick={handleEndLive}`) — son rôle se devinait, et le même bouton servait à la fois le spectateur et l'animateur.

**Ce qui a été demandé** (LV-2, point 1) : un bouton de sortie clair, avec un libellé visible sur téléphone ET ordinateur, une sortie propre sans rechargement, une preuve visible avant la production contrôlée, puis le point suivant.

**Ce qui a été modifié** :
- `components/SocialLive.tsx` : nouveau `handleQuitLive` — animateur (`isHost`) → `handleEndLive()` **inchangé** (compte-rendu de fin conservé) ; spectateur → `stopLocalMedia()` + `leaveRealSession()` (`left_at` posé) + `onClose()` (changement d'état côté `App.tsx`, jamais un rechargement de page) ; bouton d'en-tête relibellé « Quitter » (`data-testid="live-quit-button"`, `aria-label="Quitter le direct"`, 44 px, `shrink-0` hors de la zone défilante — acquis MB-1).
- `tests/liveQuitButton.test.tsx` (nouveau) : garde-fou de structure en CI (libellé visible, `data-testid`, `aria-label`, câblage `handleQuitLive`, absence de `location`/`reload`).

**Preuves** : typage 0, 1635/1635 tests (108 fichiers), build propre, banc navigateur réel avant/après 12/12 (ordinateur 1440×900 + téléphone 390×844) ; Green Gate vert sur `97abca9` puis sur `main` (run #253). Bundle servi revérifié le 5/09 après l'avance de `main` (PR #114, PR #111) : `moknet.net` sert `index-C8Y6seFM.js` (marqueurs `live-quit-button` et « Quitter le direct » présents, ancien libellé « Quitter ou terminer le Live » absent), bundle de la fusion `index-C_IAplN2.js` → 404. Aucune migration, aucune fonction Edge, aucune donnée touchée. Le vécu complet (entrer → cliquer → sortir sans rechargement) et l'appréciation visuelle finale appartiennent à la Direction.

### [Version 6.42.1] — 5 Septembre 2026 (Architecte : plus de bande sombre au sommet du cadre rond) — EN PRODUCTION CONTRÔLÉE
- **Constat de la Direction** (point orange du rapport de la v6.41.0) : une bande sombre au sommet du cadre rond de `/architecte`. **Cause mesurée** : la photo validée n'a pas de marge au-dessus du crâne ; `drawFramed` laissait le débord du cadre transparent et l'export JPEG le rendait noir (95 px sur 768, 12,4 %) ; le modèle HeyGen généré depuis ce portrait portait la même bande (89 px sur 720), l'affiche aussi.
- **Remède dans le moteur** (`photoAvatarEngine.ts`) : le débord est comblé sous la photo par un prolongement adouci du fond (bords reflétés, flou par sous-échantillonnage, fondu vers la teinte du bord), canvas 2D portable ; le masque de silhouette garde son débord transparent. **Avertissement** nommant le côté dès 2 % de débord (`overflowBands`, `describeOverflow`, testés) : « le cadre déborde de la photo en haut (12,5 %) ».
- **Actifs régénérés par le même moteur** dans Chromium réel : `architecte.webp` (cadrage identique, calage d'usine conservé), `architecte-silhouette.png` ; vidéo plein cadre recomposée sur le portrait corrigé à travers la silhouette du détourage (aucun crédit HeyGen), `.mp4` / `.webm` / affiche ré-encodés aux mêmes réglages ; vidéos détourées inchangées ; registre des séquences (empreintes, tailles) à jour et vérifié par les tests.
- **Revue indépendante (contrôleur séparé) avant fusion** : huit constats, tous traités — filet sombre résiduel de 4 px dans la vidéo et l'affiche (la rampe de fondu mordait sur la bande ; désormais poids 1 jusqu'à la ligne 92, 0 à 112, écart ligne à ligne < 1,5 entre 10 et 14 % de la hauteur), socle du prolongement (aucune zone transparente même sur une photo coupée très serrée), géométrie des tuiles extraite en fonction pure testée, empreintes du portrait, de la silhouette et de l'affiche signées par `tests/architecteAssets.test.ts`, captures vidéo avec le sommet du cadre à l'écran, chiffres de preuve de l'arbre livré, numéro v6.42.1 (v6.42.0 prise par `main` avant la livraison).
- **Preuves** : typage 0, 1642/1642 tests (109 fichiers), build ✓ ; `docs/captures/2026-09-05-architecte-bande-sombre/` (captures avant/après ordinateur, tablette, téléphone, au repos et vidéo en lecture ; luminance de la bande 0 → 131–136 pour le portrait et 0 → 132 pour la vidéo ; profil des fichiers livrés sans filet ; planche portrait / silhouettes ; empreintes ; captures de production « avant » à 16:04 UTC).
- **Production contrôlée** (feu vert de la Direction : « tu montres une preuve avant-après sur mobile et ordi, puis seulement après tu mets en production contrôlée ») : PR #111 fusionnée en squash → `main` `f6d9a16` le 5/09/2026 à 17:47 UTC après Green Gate vert sur `d5550ed` (run 33981843750), `main` vérifié inchangé (`e9595e5`) à 17:46:58 UTC et contre-vérification « PRÊT » ; `moknet.net` sert `index-C8Y6seFM.js` depuis 17:48 UTC, actifs corrigés servis aux tailles exactes, ancien bundle `index-C_IAplN2.js` → 404 ; fumée Chromium sur la page servie : bande à 132 (portrait et vidéo) sur ordinateur et téléphone ; Green Gate sur `main` run 33982039577 ; captures de production « après » versionnées (`production-apres-*-17h48.jpg`, `production-apres-fumee-17h48.json`, `production-apres-verification-17h48.txt`).
### [Version 6.42.0] — 5 Septembre 2026 (Accès public : verrou d'entrée vérifié par le serveur, domaine canonique)
**Référence** : DEC-2026-081 · branche `claude/moknet-public-access-auth-hu6yy7` · statut 🟢 en production contrôlée depuis le 5/09/2026 à 17:01 UTC (PR #105 → `main` `daa6575`, `moknet.net` sert `index-CacRDIgE.js`).

**Ce qui existait** : l'entrée de l'application relisait la session gardée par l'appareil (`getSession()`, stockage local) et ouvrait l'interface dès qu'une session existait — y compris quand le chargement du profil échouait (« Local-First », v6.1). Le serveur d'authentification n'était jamais consulté à l'ouverture.

**Ce qui a été demandé** : accès direct à l'interface strictement réservé aux sessions valides, écran de connexion ou de création de compte pour tout le monde sinon, quel que soit le canal d'ouverture du lien, toutes les écritures du domaine ramenées à `moknet.net` en minuscules.

**Ce qui a été prouvé avant de coder** : sur la page de production servie, en navigateur vierge, l'écran de connexion s'affiche sur les sept canaux ; en revanche une session locale que le serveur refuse (401/403 rejoué) ouvrait l'interface interne partout.

**Ce qui a été modifié** :
- `services/auth.ts` : `verifierSession()` — `GET /auth/v1/user` avec le jeton local ; verdict `valide` / `invalide` (refus 401/403 de Supabase lui-même : session effacée par `signOut({ scope: 'local' })`) / `non-verifiee` (aucun verdict : panne réseau, 5xx, 429, réponse non JSON d'un intermédiaire, 8 s sans réponse — session locale non expirée conservée, tolérance dite).
- `App.tsx` : toute session, quel que soit l'événement (`getSession()`, `INITIAL_SESSION`, `SIGNED_IN`, `TOKEN_REFRESHED`, `USER_UPDATED`) → verdict de son jeton, une seule vérification par jeton, un jeton refusé reste refusé (supabase-js rejoue la session du stockage en `SIGNED_IN` au retour sur l'onglet et entre onglets — constat du contrôle indépendant) ; garde contre la course avec une déconnexion pendant le chargement du profil. Aucun écran, aucun gestionnaire retiré.
- `netlify.toml` : `https://www.moknet.net/*`, `http://www.moknet.net/*`, `http://moknet.net/*` → `https://moknet.net/:splat` (301, forcées, avant les réécritures de chemin) ; garde-fou écrit contre la boucle si le domaine principal change.
- Tests : `tests/sessionValidee.test.ts` (12), `tests/appEntreeReseau.test.tsx` (+12, dont le rejeu `SIGNED_IN` et la course avec une déconnexion), `tests/netlifyRedirectsCanoniques.test.ts` (6). Revue indépendante « À CORRIGER » puis contre-vérification « PRÊT » (quatre mineurs corrigés dans la foulée).

**Résultat mesuré** (`docs/captures/2026-09-05-acces-public-authentification/`, 70 mesures) : session refusée par le serveur — 7/7 interface interne avant, 7/7 écran de connexion après ; navigateur vierge — 28/28 écran de connexion ; session confirmée — 14/14 Réseau MokNet ; dix écritures du domaine tracées, toutes sur `https://moknet.net/`.

### [Version 6.41.1] — 5 Septembre 2026 (Assistant IA Pré-Publication : « Appliquer à ma publication » au-dessus du dock sur téléphone) — EN PRODUCTION CONTRÔLÉE

* **La demande** (Direction, 5 septembre, capture iPhone) : « sur téléphone,
  dans le parcours de publication, le bouton Appliquer à ma publication est
  masqué par la barre du bas où il y a menu et messages. Il faut que ce
  bouton reste visible et cliquable sur mobile. […] Je veux une correction
  prouvée sur téléphone, avec un parcours complet jusqu'à la publication. »
* **Ce que le membre voit désormais sur téléphone** : la modale de
  l'assistant IA couvre tout l'écran, barre du bas comprise ; le pied
  « Annuler | Appliquer à ma publication » est dégagé et répond au doigt ;
  le texte optimisé revient dans le composeur, « Publier » publie. Rien n'a
  changé dans la modale (onglets, tons, textes, boutons, matière, dégradés) ;
  sur ordinateur, même modale, simplement centrée sur la fenêtre.
* **Comment** : la modale est portée sur `<body>` (portail React) hors de
  `#root`, qui devient inerte pendant l'ouverture ; feuille : bloc
  « ASSISTANT IA — MODALE AU-DESSUS DU DOCK » (`z-index` 2147482000 sous le
  studio Visuel IA, hauteur `90dvh` avec repli `90vh`, pied avec zone sûre) ;
  `data-miroir` sur la modale pour garder l'habillage aqua ; dialogue
  accessible (`role="dialog"`, `aria-modal`, focus, Échap). Composant
  `components/AIPostAssistantModal.tsx`, logique IA inchangée.
* **Preuves** : typage 0 erreur, 1602/1602 tests (105 fichiers, 7 nouveaux),
  build OK ; parcours complet rejoué avant / après (script versionné, JSON,
  captures) sur 390 × 844, 360 × 800 et 1440 × 900 :
  `docs/captures/2026-09-05-assistant-ia-modale-dock/`. Production : PR #109
  fusionnée en squash `81c66c8` à 16:46:29 UTC sur feu vert écrit, page servie
  par `moknet.net` depuis 16:47:11 UTC (bundle `index-6ZrCib2c.js`), Green
  Gate vert sur `main` (DEC-2026-080).

### [Version 6.41.0] — 5 Septembre 2026 (La photo validée devient l'avatar vivant de l'Architecte) — EN PRODUCTION CONTRÔLÉE
- **Portrait d'usine = la photo validée par la Direction** (reçue le 05/09 à 14:50 UTC), cadrée et mesurée par le moteur MediaPipe de l'option Super-Admin (478 repères, aucun avertissement) : `architecte.webp` 768 px, masque de silhouette 512², rig et ancre de bouche d'usine ; règle `FACTORY_PORTRAIT_ID` (un calage hérité d'un ancien portrait d'usine est écarté ; une photo déposée garde le sien).
- **Une seule voix** : phrase officielle enregistrée avec la voix attitrée de l'Architecte (George, ElevenLabs multilingual v2), servie par `/architecte/vision-smart.wav` ; l'ancienne voix « Claire » n'est plus qu'un banc de mesure de l'aligneur (`tests/fixtures/`).
- **Modèle vidéo HeyGen depuis la photo validée** (9,08 s, 720 × 720, 4 crédits : 563 → 559), détouré image par image (228 images), légendes sur l'aligneur du produit, calage vidéo ↔ portrait par le moteur de production ; empreintes vérifiées par les tests.
- **Super-Admin** : onglet dédié « Avatar de l'Architecte » (2ᵉ onglet), enregistrement immédiat et horodaté, rechargement prouvé, retour arrière ; note de renvoi dans « Paramètres Plateforme ».
- **Réglage partagé** : `platform_settings` (clé `architecte_avatar`) lu au démarrage et écrit à chaque enregistrement, dégradation silencieuse sans table ; migration + retour arrière versionnés, **non appliqués** (feu vert explicite requis).
- **Zéro obstruction** : la sculpture se retire en pastille pendant la saisie dans MokNet (mesuré sur le fil réel : « Publier » 0 px² couvert à 390 × 844 et 360 × 800 pendant la saisie, contre 2 301 / 2 419 px² au repos).
- **Preuves** : typage 0, 1595/1595 tests (104 fichiers), build ✓, `docs/captures/2026-09-05-architecte-photo-validee/` (23 fichiers : captures Super-Admin, application, fil réel, mesures du moteur, empreintes, scripts).

### [Version 6.40.0] — 5 Septembre 2026 (Réseau MOC : même disposition des boutons sur ordinateur et téléphone) — EN PRODUCTION CONTRÔLÉE
...
* **Ce que le membre voit désormais sur téléphone** : dans « créer une
  ...
  Voix **avec ses libellés** juste dessous, le champ à droite (249 px sur
  ...
  rien ne change (mesures identiques). Sur une carte très étroite (≤ 270 px
...
* **Comment** : CSS seulement (blocs « COMPOSEUR A7 » et « BANDE AURORE »
  ...
  même boîte et les mêmes coordonnées sur les six écrans mesurés.
* **Preuves** : typage 0 erreur, 1589/1589 tests (104 fichiers) (gardes CSS mises à jour :
  rail non masqué à 560 px, rangées `auto 1fr`, quatre colonnes IA, repli
  270 px, replis 639/344 ; bande en grille 4 colonnes sans `overflow-x`,
  `scroll-snap` ni `mask-image`), build OK, captures et mesures avant/après
  sur 1440 × 900, 820 × 1180, 390 × 844, 360 × 800, 375 × 667 et 320 × 568
  (`docs/captures/2026-09-05-reseau-meme-disposition-telephone/README.md`).
  Voir DEC-2026-078. Production : feu vert écrit du 5/09 (vers 15:11 UTC), fusion en squash à 15:12:30 UTC (`a615593`), page servie par `moknet.net` à 15:13:19 UTC, Green Gate vert sur `main`.
...
* **Contrôle indépendant** : revue de code indépendante avant fusion —
  un bloquant (studio remis à zéro par un re-rendu du parent), quatre
  importants (requête de conteneur morte sur tablette / téléphone, réponse
  IA vide prise pour un succès, focus non rendu, export vidéo Safari) et
  quinze mineurs ; les cinq premiers et dix mineurs corrigés puis couverts
  par des tests, détail dans DEC-2026-061.
* **Preuve** : typage 0 erreur ; 1233/1233 tests (86 fichiers, dont 17 dans
  `tests/composeurRail.test.tsx`, 19 dans `tests/visuelIA.test.ts`, 21 dans
...
* **Production** : PR #93 fusionnée en squash → `main` `36e0a44` le
  5/09/2026 à 12:38 UTC, sur feu vert écrit de la Direction, après Green
  Gate vert sur `94514df` et contre-vérification indépendante « PRÊT » ;
  `moknet.net` sert `index-ggiwFepQ.js` depuis 12:38:39 UTC (même bundle
  que le preview contrôlé), ancien bundle 404, blocs CSS et marqueurs
  présents, miroir local vérifié dans Chromium. Le contrôle visuel final
  dans l'application appartient à la Direction.
...
* **Mise en production contrôlée** (validation de la Direction le 5/09/2026 à
  ...
  **geste réel « Clore » joué à 10:55 UTC** sur un direct de banc privé :
  ...
  de bord réel consignés par la présente PR de mémoire vivante : le client SAT-6 est porté depuis `4486cc4` dans le bundle courant de `moknet.net` (empreintes SAT-6 présentes dans le bundle servi à ce jour, ancien `index-BKkSLQXf.js` de la fusion → 404) ; panneau « Secours du direct » prouvé visible et actif dans le tableau de bord réel (banc navigateur 11 OK / 0 DÉFAUT sur la source servie par la production : gestes « Relancer »/« Clore » actifs pour un `super_admin`, « Lecture seule » sans bouton pour un `admin`, panneau absent pour un `user`).
...
* **Production** : oui, contrôlée — feu vert écrit de la Direction, PR #83
  fusionnée en squash (`5b1c1ce`) après Green Gate vert sur la tête finale
  `af6a8d2` ; `moknet.net` sert `index-BUCPWfy5.js` depuis le 5 septembre à
  07h20 UTC (bloc « PLATEAUX DE CRISTAL (DEC-2026-056) » dans la page,
  marqueurs du panneau présents, anciens textes absents, ancien bundle
  `index-CjAVWgcX.js` en 404) ; miroir local ouvert dans Chromium : racine
  montée, règles et six `@keyframes cristal-*` analysées, aucune erreur JS
  applicative. Contrôle visuel final dans l'application par la Direction.
