...
- **Remèdes** (même périmètre, rien retiré) : `voiceEngine.unlockPlayback()` appelé DANS les gestes (appui sur la sculpture, bouton Présentation, envoi clavier, ouverture) — démarre le contexte de sortie et « bénit » UN élément audio réutilisé pour toute la voix HD (clip WAV silencieux construit en mémoire) ; `playAudioUrl` attend un contexte réellement `running` (borné à 150 ms) et ne relie l'élément au graphe QUE si le contexte tourne, sinon la voix sort en direct et la bouche suit la piste phonétique ; première phrase refusée → `finishSpeakingElevenLabs(…, 'relais')` + repli navigateur (`onEnd` une seule fois, micro non relancé entre les deux) ; flux de capture micro relâché pendant la lecture (téléphone : plus de sortie en mode « appel ») ; sculpture ouverte = `reprendreLaParole` (se tait s'il parlait, coupe la présentation si elle jouait, « Je vous écoute. », écoute) et libellé « Parler à l'Architecte » — fermer reste sur le ✕ « Fermer » ; `armListenWatchdog` patiente tant que la voix parle ; `dire()` + jeton de réflexion (`beginThinking` / `settleThinking`, `onStart` de la voix) ; époque `speakEpoch` vérifiée par toutes les minuteries de la file navigateur.
- **Tests** : `tests/voiceEngine.test.ts` (+5 : déverrouillage idempotent, élément réutilisé relié UNE fois au graphe quand le contexte tourne, contexte suspendu → élément non relié et voix en direct, première phrase refusée → repli et `onEnd` unique, flux micro relâché pendant la voix HD) ; `tests/ArchitecteFloatingBar.test.tsx` (+4 : appui sur la sculpture ouverte = se tait / écoute / déverrouille sans fermer, appui pendant la présentation = accueil puis écoute, réflexion tenue jusqu'au premier son, chien de garde qui patiente pendant la voix) ; typage 0, **1651/1651 tests (109 fichiers)**, build ✓.
...
- **Coordination** : PR à ouvrir sur la branche désignée après la fusion de la PR de documentation #116 (une seule PR par branche) ; numéro réservé DEC-2026-083 / v6.44.0 (DEC-2026-082 et v6.43.0 sont pris par les PR #115 et #63 en cours) — renumérotée si `main` en décide autrement.
### [Version 6.42.1] — 5 Septembre 2026 (Architecte : plus de bande sombre au sommet du cadre rond) — EN COURS DE PRODUCTION CONTRÔLÉE
