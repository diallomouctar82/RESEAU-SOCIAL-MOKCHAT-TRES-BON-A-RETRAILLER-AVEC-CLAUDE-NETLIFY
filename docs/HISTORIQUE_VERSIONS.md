...
| **v6.44.0** | 5 Septembre 2026 | **Architecte : l'avatar accompagne TOUTE la conversation — après sa présentation il dit l'accueil, écoute, dit chaque réponse par la voix attitrée avec la bouche synchronisée, et repart en écoute ; toucher l'avatar pendant qu'il parle = « je vous écoute » (plus une fermeture) ; lecture audio déverrouillée dans le geste (téléphone), jamais un silence pris pour un succès, chien de garde du micro qui patiente pendant la voix, réflexion tenue jusqu'au premier son** | Diallo OS & Architecte (module 01) : barre flottante (`ArchitecteFloatingBar.tsx`) ; moteur vocal partagé (`services/voiceEngine.ts` — lecture HD, repli, file navigateur) ; tests `tests/voiceEngine.test.ts`, `tests/ArchitecteFloatingBar.test.tsx` | branche `claude/moknet-studio-avatar-rjvvip` / DEC-2026-085 (numéro réservé : DEC-2026-083, 084 et v6.43.1 sont pris par la PR #117 en cours) | **Prête pour revue indépendante et production contrôlée — aucune production sans feu vert écrit** : typage 0 erreur, 1651/1651 tests (109 fichiers), build ✓ ; banc Chromium avant/après sur ordinateur 1440 × 900 et téléphone 390 × 844 (`docs/captures/2026-09-05-architecte-continuite/`) : 3 paroles HD entendues, bouche ouverte sur 71 % des images pendant la parole et 0 hors parole, 1 seule liaison au graphe audio (5 avant), appui pendant la réponse → barre ouverte et écoute (avant : tout fermé) ; PR à ouvrir après la fusion de la PR de documentation #116 (branche désignée occupée). |
...
### [Version 6.44.0] — 5 Septembre 2026 (Architecte : l'avatar accompagne toute la conversation) — PRÊTE POUR REVUE ET PRODUCTION CONTRÔLÉE
- **Constat de la Direction** (production réelle, téléphone et ordinateur, 05/09) : « L'avatar est bien là, visible et vivant, mais après sa présentation, il ne porte plus la conversation. Il faut que l'avatar accompagne toute la conversation et soit synchronisé aux réponses. Corrige cela dans le même périmètre. »
- **Causes mesurées** (audit de `ArchitecteFloatingBar.tsx` et `voiceEngine.ts`) : (1) la voix HD reliait chaque phrase (`new Audio()` par phrase) au graphe d'un contexte audio créé HORS geste — resté `suspended` (Safari/iOS refuse `resume()` hors geste), l'élément « jouait » jusqu'au bout sans qu'aucun son sorte et sans que l'analyseur ne voie rien : Architecte muet et immobile, aucune erreur ; (2) une première phrase refusée à `play()` rendait `true` — silence pris pour un succès, aucun repli ; (3) la sculpture ouverte était le bouton qui FERME : toucher l'avatar pendant la conversation coupait écoute, voix et session ; (4) le chien de garde du micro (3 s) affichait « Le micro n'a pas démarré » et passait l'avatar en erreur pendant que la voix parlait (elle met le micro en pause exprès) ; (5) la réflexion s'éteignait dès l'envoi de la réponse, avant le premier son (1 à 4 s de génération HD) — l'avatar retombait au repos ; (6) minuteries de la file navigateur (différé 80 ms, respiration, chien de garde) non liées à l'époque de la parole — une minuterie périmée pouvait piloter la file suivante (défaut latent vu au banc).
- **Remèdes** (même périmètre, rien retiré) : `voiceEngine.unlockPlayback()` appelé DANS les gestes (appui sur la sculpture, bouton Présentation, envoi clavier, ouverture) — démarre le contexte de sortie et « bénit » UN élément audio réutilisé pour toute la voix HD (clip WAV silencieux construit en mémoire) ; `playAudioUrl` attend un contexte réellement `running` (borné à 150 ms) et ne relie l'élément au graphe QUE si le contexte tourne, sinon la voix sort en direct et la bouche suit la piste phonétique ; première phrase refusée → `finishSpeakingElevenLabs(…, 'relais')` + repli navigateur (`onStart` et `onEnd` une seule fois ; sans synthèse navigateur : message honnête, `onEnd` et écoute relancée — `abandonSpeech`) ; iPhone / iPad : voix en direct, jamais reliée au graphe Web Audio (interrupteur silencieux) ; sculpture ouverte = `reprendreLaParole` (se tait s'il parlait, coupe la présentation si elle jouait, « Je vous écoute. », écoute) et libellé « Parler à l'Architecte » — fermer reste sur le ✕ « Fermer » ; `armListenWatchdog` patiente tant que la voix parle ; `dire()` + jeton de réflexion (`beginThinking` / `settleThinking`, `onStart` de la voix) ; époque `speakEpoch` vérifiée par toutes les minuteries de la file navigateur.
- **Tests** : `tests/voiceEngine.test.ts` (+9 : déverrouillage idempotent et un seul clip pour deux gestes, élément réutilisé relié UNE fois au graphe quand le contexte tourne, contexte suspendu → élément non relié et voix en direct, refus mémorisé jusqu'au geste suivant, première phrase refusée → repli et `onEnd` unique, relais sans synthèse → `onEnd` + message honnête + micro relancé, `onStart` unique, iPhone → jamais relié) ; `tests/ArchitecteFloatingBar.test.tsx` (+4 : appui sur la sculpture ouverte = se tait / écoute / déverrouille sans fermer, appui pendant la présentation = accueil puis écoute, réflexion tenue jusqu'au premier son, chien de garde qui patiente pendant la voix) ; typage 0, **1651/1651 tests (109 fichiers)**, build ✓.
- **Preuves** : `docs/captures/2026-09-05-architecte-continuite/` — même scénario avant (`origin/main` `f6d9a16`) et après, ordinateur et téléphone, voix HD doublée par un extrait réel de la voix attitrée : après, 3 paroles HD entendues, bouche ouverte sur 71 %/72 % des images pendant la parole et 0 hors parole, 1 liaison au graphe (5 avant), appui pendant la réponse → barre ouverte, « Je vous écoute. », écoute (avant : tout fermé, mode conversationnel coupé). Limite dite : le mutisme par contexte suspendu (Safari/iOS) n'est pas reproductible en Chromium headless — couvert par les tests du moteur ; le contrôle sur téléphone réel appartient à la Direction.
- **Revue indépendante** (contrôleur séparé) : « À CORRIGER » sur `50f481c` — 2 IMPORTANT, 9 MINEUR — tous traités dans `c396623` (voir DEC-2026-085) ; contre-vérification « PRÊT » sur `0604be6`, quatre mineurs à faible coût traités dans la foulée. La sculpture ouverte ne referme plus (« Parler à l'Architecte » ; fermer = ✕) : à contrôler par la Direction.
- **Coordination** : `main` réintégré (PR #115 v6.43.0, #118, #116) sans re-couronner ; numéro réservé DEC-2026-085 / v6.44.0 (DEC-2026-083, 084 et v6.43.1 pris par la PR #117 en cours) — renumérotée si `main` en décide autrement.
