...
| **v6.44.0** | 5 Septembre 2026 | **Serveur d'authentification injoignable : fermé par défaut — écran de reprise « MokNet est momentanément injoignable, veuillez réessayer. », session locale conservée si elle n'a pas été refusée, reprise sans ressaisie dès que le serveur répond (bouton, retour du réseau, retour sur la page, minuterie 30 s), relecture plafonnée à 8 s ; `/architecte` confirmée publique après audit (démonstration sans donnée ni fonction sensible)** | Authentification (`services/auth.ts#relireSession`, `App.tsx`, nouveau `components/ServeurInjoignable.tsx`), tests (`appEntreeReseau` 28, `sessionValidee` 16) | Vision Smart — Direction 05/09/2026 (décision écrite sur DEC-2026-081) · DEC-2026-083 | **En PR (#120, brouillon) — banc, revue indépendante et contre-vérification en cours ; aucune production sans feu vert écrit de la Direction** |
...
### [Version 6.44.0] — 5 Septembre 2026 (Serveur d'authentification injoignable : fermé par défaut, écran de reprise, reprise sans ressaisie)
**Référence** : DEC-2026-083 · décision écrite de la Direction du 05/09/2026 · branche `claude/moknet-public-access-auth-hu6yy7` · statut 🟠 en PR #120 (brouillon), Green Gate et revue en cours.

**Ce qui existait** (v6.42.0, DEC-2026-081) : toute session relue depuis l'appareil était vérifiée par le serveur avant l'interface ; sans verdict (réseau coupé, 5xx, 429, 8 s sans réponse), la session locale non expirée ouvrait l'interface (tolérance dite), vide et sans données ; une session expirée hors ligne laissait une roue de chargement 25 s (reprises internes de supabase-js) puis l'écran de connexion.

**Ce qui a été demandé** (Direction, 05/09) : ne pas ouvrir l'interface quand la session ne peut pas être vérifiée ; message clair de reprise « MokNet est momentanément injoignable, veuillez réessayer. » ; ne pas effacer la session locale si elle n'a pas été refusée ; reprise sans ressaisie dès que le réseau revient ; `/architecte` publique seulement si démonstration sans donnée ni fonction sensible.

**Ce qui a été modifié** :
- `services/auth.ts` : `relireSession()` (« session » / « aucune » / « injoignable ») ; `getSession()` et `verifierSession()` inchangés.
- `components/ServeurInjoignable.tsx` (nouveau) : écran de reprise, message exact, bouton « Réessayer » ≥ 44 px avec état de tentative, rappel des reprises automatiques, détail technique.
- `App.tsx` : `non-verifiee` → écran de reprise (jamais l'interface, jamais `signOut`) ; verdict non vérifié jamais mémorisé ; relecture initiale plafonnée à 8 s, résultat tardif appliqué ; tentative unique partagée (relecture initiale, bouton, `online`, minuterie 30 s, tentative programmée après la fenêtre de 60 s du cache d'échec de supabase-js) ; au retour sur la page, rejeu `SIGNED_IN` de supabase-js par le même verrou ; `INITIAL_SESSION` nul hors ligne ne ferme pas l'écran, `SIGNED_OUT` / déconnexion / refus le ferment.
- `/architecte` : audit du 05/09 — configuration de l'avatar en mémoire (valeurs livrées ; la table `platform_settings` n'existe pas dans le projet Supabase), fichier audio statique, voix du navigateur, aucune écriture, aucun service payant, aucune donnée de compte → reste publique (condition de la Direction remplie).

**Preuves** : typage 0, 1 658/1 658 tests (109 fichiers), build propre ; banc navigateur sur le build de la branche : **78/78 mesures conformes** (26 scénarios × 3 supports, build corrigé de la tête `93a1aad`, `docs/captures/2026-09-05-serveur-injoignable-reprise/`) — serveur coupé avec session non expirée → écran de reprise ; retour du réseau (`online`), retour sur la page (rejeu `SIGNED_IN` de supabase-js) ou bouton → interface en ≈ 1,1 s, jeton conservé, sans ressaisie ; serveur toujours injoignable → écran maintenu ; refus à la reprise → effacement + connexion ; session expirée hors ligne → écran de reprise en 8,1 s ; réseau revenu pendant les reprises internes de supabase-js → interface en ≈ 4,1 s ; réseau revenu 35 s après l'ouverture (reprises épuisées, cache d'échec de 60 s) → interface ≈ 42 s après le retour du réseau, sans ressaisie ; tous les cas de DEC-2026-081 (connecté, déconnecté, sans compte, navigation privée, session invalide/expirée, liens privés, `/architecte`) inchangés ; revue indépendante (agent séparé, producteur ≠ contrôleur) sur `9d4d957`→`b4b23d0` : « À CORRIGER », aucun bloquant — C1 IMPORTANT (cache d'échec de 60 s de supabase-js : reprise d'une session expirée non immédiate au retour du réseau), C2 IMPORTANT (preuves P3 absentes de la tête revue), C3–C13 mineurs (double écouteur de visibilité, tentative et relecture non partagées, promesses sans `.catch`, constante entre imports, contraste du détail technique, `role="alert"` englobant le bouton, focus perdu sur `disabled`, texte « 30 secondes » en dur, cas de test manquants, formulation `/architecte`, limite `/token`) ; toutes corrigées sur `93a1aad` (tentative programmée après la fenêtre du cache avec garde-fou sur la constante de auth-js, tentative et relecture uniques, écran accessible, 7 tests ajoutés, documents précisés) ; contre-vérification en cours sur la tête finale.
