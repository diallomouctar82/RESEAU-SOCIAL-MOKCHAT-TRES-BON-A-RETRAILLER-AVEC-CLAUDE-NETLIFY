...
### [DEC-2026-086] — 5 Septembre 2026
* **Module(s)** : Sécurité & infrastructure (`kit-sauvegarde/`, fiche `docs/modules/14_securite_et_infrastructure.md`) ; garde-fous de test (`tests/kitSauvegarde.test.ts`). Aucun code livré modifié.
* **Problème / Besoin initial** : Direction (05/09) : « Prépare un kit global de sauvegarde et redéploiement pour MokNet. Le but est d'avoir un fichier unique qui permette de tout reconstruire ailleurs. Le dépôt git complet avec historique, le code source sans clés, les paramètres de build, un schéma des variables d'environnement sans valeurs secrètes, et un script qui demande les clés au moment du redéploiement et les installe automatiquement. Ajoute aussi la structure Supabase sous forme de migrations et guides, sans données sensibles. Crée un fichier qui décrit l'état exact de l'application au moment de la sauvegarde. […] Livrable : un zip de redéploiement complet, testé en local, avec preuves. » Puis : « créer un assistant guidé qui accompagne le redéploiement étape par étape […] afficher l'état de l'appli, puis demander les clés une par une au bon moment. Supabase, Netlify, GitHub si besoin […] placer ces clés automatiquement aux bons endroits […] reconstruire la partie Supabase, tables, structure, migrations, puis enchaîner sur Netlify […] S'il y a d'autres fournisseurs, le système les détecte et demande ce qu'il faut. Un seul parcours guidé, propre. » Et : « Travail en parallèle, mais sans marcher sur les autres […] branche dédiée […] tu capitalises sur ce qui existe et tu complètes. » **Constat d'audit** : aucune sauvegarde reconstructible n'existait ; le dépôt ne versionne que 6 fichiers de migration alors que la base de production en porte 110 (versions et instructions relevées dans `supabase_migrations.schema_migrations`) ; `docs/supabase_schema.sql` est marqué obsolète ; `netlify.toml` n'a volontairement pas de section `[build]` ; aucune référence au projet Supabase en dur dans le code applicatif (uniquement docs et tests) ; le domaine `moknet.net` est en dur dans six fichiers de code ; `pg_cron` et le bucket `mok bouker` existent en production sans migration.
* **Idées / Options envisagées** : (1) `pg_dump` complet — refusée : exige le mot de passe de la base et emporterait des données réelles, hors périmètre « sans données sensibles » ; (2) copier les 6 migrations du dépôt comme structure — refusée : incomplète (110 versions en base), horodatages différents ; (3) scripts bash séparés (sauvegarde, puis redéploiement à la main) — refusée après la consigne « un seul parcours guidé » ; (4) **retenue** : un dossier autonome `kit-sauvegarde/` (Node ≥ 22, zéro dépendance) avec un créateur de zip, un vérificateur et un assistant guidé en treize étapes ; structure Supabase relevée en lecture seule sur la base réelle (catalogues Postgres et historique des migrations, jamais `vault.decrypted_secrets`), embarquée comme historique complet + prérequis + compléments + guide.
* **Décision retenue — kit global en un zip et assistant guidé unique** : `creer-kit.mjs` produit `moknet-kit-<date>-<sha7>.zip` (+ `.sha256`) : bundle git complet vérifié (toutes les branches locales et distantes connues, étiquettes, historique), archive `git archive` du commit (donc jamais de `.env`), paramètres de build (`netlify/parametres-build.json` + `netlify.toml`), schéma des variables sans valeurs (`env/schema-env.json` → `SCHEMA_ENV.md`, par fournisseur : où la clé est demandée, où elle est installée, par quoi elle est lue), structure Supabase (`supabase/migrations/` 110 fichiers, `prerequis.sql`, `complements-hors-migrations.sql`, `releve/*.json`, `STRUCTURE.md` générée, `extraire-structure.sql`, `GUIDE_SUPABASE.md`, fonctions Edge), état exact mesuré (`ETAT_SAUVEGARDE.md` + `manifeste.json`), sommes de contrôle, l'assistant. Garde-fous : fichier `.env` interdit, scan anti-secrets bloquant (motifs JWT, `sk-`, Google, GitHub, `sbp_`, `sb_*`, Netlify, clé privée, secret en dur, littéral du coffre, URL avec identifiants ; doublures de tests, docs, maquettes et valeurs qui se déclarent factices en avertissement seulement), `git bundle verify`, `unzip -t`. `redeployer.mjs` : 0 vérification du kit et **affichage de l'état** ; 1 prérequis ; 2 restauration depuis le bundle (commit, références, fichiers vérifiés) ; 3 GitHub facultatif ; 4 projet Supabase cible (**production refusée** sans `--autoriser-production` + PRODUCTION tapé) ; 5 prérequis → 110 migrations une par une dans leur transaction avec versions enregistrées → compléments → vérification ATTENDU/OBTENU des comptes ; 6 fonctions Edge (API de gestion, JWT vérifié) puis leurs secrets demandés un par un ; 7 authentification (URL du site, redirections, Google, URI de rappel affichée) ; 8 coffre LiveKit facultatif ; 9 clé anon récupérée par l'API et `.env` écrit (0600, ignoré par git vérifié) ; 10 `npm ci` → `tsc` → `npm test` → `npm run build` ; 11 Netlify (site, paramètres de build posés par l'API, variables, aperçu, production **seulement si PRODUCTION est tapé**) ; 12 détection des fournisseurs non répertoriés (`import.meta.env.*`, `Deno.env.get`) et placement selon la portée, clés IA à saisir dans Super Admin, promotion du premier Super Admin ; 13 vérification finale et rapport ATTENDU/OBTENU/ÉCART/VERDICT. Modes `--simulation`, `--reponses`, `--reprendre`, `--mode-migrations api|cli`. Un secret ne passe jamais en argument, n'est jamais journalisé (masqué), n'est écrit que dans sa cible ; rapport et état de reprise sans secret.
* **Justification & Valeur ajoutée** : reconstruction possible ailleurs à partir d'un seul fichier, sans dépendre d'une machine ni d'une mémoire ; l'historique Supabase de référence est enfin versionné (110 versions, mêmes numéros que la production) ; chaque clé est demandée au moment où elle sert et posée là où le code la lit, ce qui supprime la reconstitution à la main ; principes repris de l'épreuve de restauration Vision Smart ERP (16/08/2026) : le socle porte la forme, refus explicites, empreintes par fichier, nombres mesurés, limites dites.
* **Conséquences & Impacts transversaux** : aucun fichier livré modifié ; `supabase/migrations/` et `package.json` intacts ; nouveau dossier `kit-sauvegarde/` et nouveau test `tests/kitSauvegarde.test.ts` (garde en CI : toute variable lue par le code doit être répertoriée dans le schéma ; historique embarqué complet et ordonné ; `pg_cron` créé avant les migrations qui le planifient ; scanner qui sait rougir ; aucun secret dans le kit). Le zip produit se conserve **hors du dépôt** (stockage chiffré de la Direction). Retour arrière : suppression du dossier et du test.
* **Contrôle indépendant** : suite complète 1653/1653 (110 fichiers, dont `kitSauvegarde` 11/11), typage 0, build ✓ ; test de bout en bout `kit-sauvegarde/tests/test-kit-local.sh` (🧪 banc local) : création du zip, extraction dans un dossier vierge, vérification (183 fichiers conformes, bundle vérifié, 0 secret), assistant en simulation avec réponses préparées et clés factices — commit restauré identique au manifeste, empreintes git de chaque fichier identiques au dépôt, `.env` écrit aux bons endroits et ignoré par git, `npm ci` / `tsc` / `build` réels, aucun secret (même factice) dans le journal, le rapport ni l'état de reprise, 0 contrôle rouge ; preuves dans `docs/captures/2026-09-05-kit-sauvegarde-redeploiement/`. Revue indépendante : voir PR.
* **Limites dites** : les étapes réseau (API de gestion Supabase, déploiement des fonctions Edge, API/CLI Netlify, `git push`) n'ont tourné qu'en simulation (⬜ dans le rapport) — leur première exécution réelle est une épreuve de restauration à mener sur un projet Supabase d'essai et un site Netlify d'essai, jamais la production ; pas de données ni d'objets de stockage ; pas de valeurs de secrets ; serveur LiveKit et réglages du tableau de bord Supabase hors catalogue non relevés ; `platform_settings` (dépôt) absente de la base au relevé ; un référentiel « AITOPIA » demandé en comparaison n'a été trouvé nulle part (dépôt, historique, dépôts accessibles, sessions, artefacts, Drive, Notion, mémoire) — comparaison faite avec l'épreuve de restauration Vision Smart, à compléter si la Direction fournit AITOPIA.
* **Production** : sans objet (outillage hors code livré) ; aucun déploiement.
* **Statut** : `Développé` · `Testé (banc local)` · 🟡 **PR brouillon en revue — épreuve de restauration réelle à planifier.**
