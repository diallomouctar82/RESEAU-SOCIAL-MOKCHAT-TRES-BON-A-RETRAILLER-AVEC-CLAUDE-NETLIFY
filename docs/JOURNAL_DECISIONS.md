...
* **Éléments techniques concernés** : `services/architecte/portraitPainter.ts` (`CANVAS_PIXEL_BUDGET`, `fitDpr`, `setQuality`, `setPixelBudget`, `desynchronized`, `dataset.resolution`), `services/architecte/livingAvatar.ts` (`adaptQuality`, `medianOf`, `speechSway`, respiration, couplage), `services/architecte/gestures.ts` (`NOD_OMEGA`, `GAZE_OMEGA`, `NOD_Y_PERCENT`, `NOD_ROLL_DEG`, partition, regard, `adoptSprings`), `components/architecte/LivingPortrait.tsx`, `ArchitecteAvatar.tsx`, `ArchitecteDemoPage.tsx` ; tests `livingAvatar` (+3), `gestures`.
