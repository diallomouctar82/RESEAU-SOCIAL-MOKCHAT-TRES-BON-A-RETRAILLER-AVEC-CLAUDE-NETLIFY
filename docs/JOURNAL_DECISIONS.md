...
* **Statut** : `Développé` · `Testé` — typage 0 erreur, **1 595 / 1 595 tests (104 fichiers)**, build ✓, bancs navigateur réel (Super-Admin ordinateur + téléphone, application ordinateur + téléphone avec la vidéo du portrait validé, fil réel 390 × 844 et 360 × 800) ; **PR ouverte — fusion en squash après Green Gate et vérification que `main` n'a pas bougé ; production contrôlée puis vérification immédiate sur `moknet.net`** ; migration `platform_settings` en attente d'un feu vert explicite ; rendu du modèle vidéo à confirmer par la Direction sur le site.
