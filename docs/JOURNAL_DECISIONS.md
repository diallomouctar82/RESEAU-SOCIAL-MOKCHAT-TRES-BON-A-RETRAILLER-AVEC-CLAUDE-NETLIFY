...
* **Module(s)** : Super-Admin — Sécurité & infrastructure (`components/AdminDashboard.tsx`, `components/admin/AdminStableVersionsTab.tsx`, note de renvoi dans `components/admin/AdminWorkflowsAndBackupTab.tsx`) ; services `services/versions/` (`versionDuCode.ts`, `stableVersions.ts`, `restaurationControlee.ts`) ; build (`vite-plugins/versionJson.ts`, `vite.config.ts`, `global.d.ts`) ; fiche `docs/modules/14_securite_et_infrastructure.md`.
* **Problème / Besoin initial** : Direction (05/09) : « créer dans le tableau de bord Administrateur Général ou Super Admin un système pour voir les versions stables et restaurer une version saine en cas de régression. […] afficher au moins les trois dernières versions stables avec nom, date, commit, PR, module, statut de prod et preuves. Bouton de restauration contrôlée, jamais à l'aveugle. En cas de problème, tu t'arrêtes, tu corriges et tu retestes. Si tu ne peux pas corriger sans risque, retour à la dernière version stable. […] Ne mélange rien hors de ce périmètre. » Puis : « tu peux travailler en parallèle, mais strictement dans ton périmètre : restauration, versions stables, tableau de bord Super Admin. Ne touche pas aux zones des autres. » Constat d'audit : le sous-onglet « Versions » de « Workflows & Sauvegarde » (v6.3) affichait des données figées et fictives (v6.3.0 « courante », sommes de contrôle inventées) et ne restaurait que la configuration locale de la console ; savoir ce que `moknet.net` sert exigeait de comparer le nom du bundle à la mémoire vivante ; aucune préparation de retour arrière n'existait dans la console.
* **Idées / Options envisagées** : (1) étendre le sous-onglet v6.3 — refusée : ses données sont fictives et son modèle (`PlatformReleaseVersion`, checksums, comptages) ne décrit pas le code livré ; le corriger reviendrait à lui faire dire autre chose que ce qu'il fait ; il reste tel quel (rien ne disparaît) avec une note de portée et un renvoi ; (2) un exécuteur serveur (fonction Edge avec jeton Netlify dans le Vault) qui republie un déploiement au clic — reportée : impossible à éprouver de bout en bout sans un jeton que seule la Direction peut créer, et un bouton dont l'effet ne peut pas être prouvé serait un faux vert ; l'onglet est conçu pour qu'un tel exécuteur se branche plus tard sans rien changer à l'écran ; (3) **retenue** : le code déclare sa propre version (`VERSION_DU_CODE`), le build écrit `/version.json` (version, commit, déploiement Netlify, bundle), l'onglet lit ce qui est réellement servi, et le bouton de restauration produit un **ordre contrôlé** — pré-contrôles, saisie exacte, motif, reconnaissance des oranges, journal nominatif partagé, procédure exacte (Netlify « Publish deploy » ou revert Git par la chaîne complète), vérification après coup — sans aucun secret dans le navigateur.
* **Décision retenue — registre des versions stables + version servie lue sur le réseau + ordre de restauration contrôlée** :
  - Registre (`REGISTRE_VERSIONS_STABLES`) : huit entrées (v6.45.0 en préparation ; v6.42.1, v6.43.0, v6.42.0, v6.41.1, v6.41.0, v6.40.1, v6.40.0 fusionnées) — chaque fait relevé dans `HISTORIQUE_VERSIONS.md` et ce journal (commit complet, PR, heure de fusion UTC, bundle à la fusion, preuves, risques : migration, données, configuration servie) ; un fait absent reste `null` et l'écran dit « non consigné ». Ordre par heure de fusion (v6.42.1 fusionnée après v6.43.0). Test `stableVersions` : chaque version du registre doit exister dans la mémoire vivante (ligne, section, commit court, PR, décision), la première entrée est `VERSION_DU_CODE`.
  - Version servie : `/version.json?v=<horodatage>` puis `/?v=<horodatage>` (le service worker sert « cache d'abord » toute requête GET de même origine hors navigation et ignore `cache: 'no-store'` — constat de l'audit, service worker non touché : zone d'une autre équipe) ; verdicts : identifiée (commit consigné, ou bundle consigné pour un déploiement antérieur au registre), déclarée (version du registre, commit non consigné : documentation seule ou code postérieur), inconnue du registre, non identifiable, injoignable.
  - Pré-contrôles : cible consignée ; cible éprouvée (tests + Green Gate ou production vérifiée) ; production lisible ; cible différente de la servie ; schéma de base (migrations introduites après la cible) ; données réelles ; configuration servie (`netlify.toml` après la cible — par exemple les redirections du domaine canonique de la v6.42.0). Un rouge refuse ; un orange exige une case cochée.
  - Ordre : `RST-AAAAMMJJ-HHMM-xxxx`, auteur (nom, e-mail, identifiant de session), cible (version, commit, bundle, PR, décision), état servi avant, motif (≥ 10 caractères), verdicts des pré-contrôles ; écrit dans `platform_settings` (clé `restauration_versions`, 50 derniers, écriture réservée aux administrateurs par la politique existante), copie locale, ligne `security` du journal d'audit de la console ; vérification après coup consignée (vert / orange / rouge).
  - Procédure : voie A Netlify (site `lovely-maamoul-478226` relevé le 05/09, déploiement du commit cible, aperçu d'abord, « Publish deploy », verrou de la publication automatique dit) ; voie B Git (`git log --oneline <cible>..origin/main`, un `git revert` par version postérieure du plus récent au plus ancien, typage + suite + build, PR, Green Gate, fusion sur feu vert).
  - Rien retiré : tous les onglets antérieurs, le sous-onglet « Versions » v6.3 et ses gestionnaires restent (garde-fou `adminDashboardOngletVersions`).
* **Justification & Valeur ajoutée** : la Direction voit en un écran les versions livrées avec leurs preuves et ce que la production sert vraiment (plus de déduction par nom de bundle) ; un retour arrière ne peut plus être improvisé : il est pré-contrôlé, nominatif, motivé, journalisé, suivi d'une vérification ; l'écran dit ce qu'il ne fait pas (aucune bascule automatique, aucun secret côté navigateur), conformément à « aucun bouton factice » et « aucun faux vert ».
* **Conséquences & Impacts transversaux** : aucune migration, aucune fonction Edge, aucune donnée touchée ; `version.json` s'ajoute au build (un commit et un identifiant de déploiement sont déjà publics : dépôt public, adresses d'aperçu Netlify) ; règle de méthode ajoutée à la compétence `production-controlee` (§ 7.2, plus strict) : toute mission qui livre du code ajoute son entrée au registre et avance `VERSION_DU_CODE` dans la même PR — sinon l'onglet dira « inconnue du registre », honnêtement. Retour arrière : `git revert` du squash (un onglet, trois services, un plugin de build, une configuration Vite).
* **Éléments techniques concernés** : voir Module(s) ; tests `tests/stableVersions.test.ts` (25), `tests/versionJsonBuild.test.ts` (4), `tests/adminStableVersionsTab.test.tsx` (11), `tests/adminDashboardOngletVersions.test.ts` (4).
* **Coordination** : `main` = `b393726` au départ ; PR ouvertes #120, #119, #117 (DEC-2026-083/084/085, v6.44.0, v6.43.1 réservés → cette mission prend DEC-2026-086 et v6.45.0, renumérotation avant fusion si `main` bouge) ; aucun fichier de code en commun avec ces PR (#63 touche `AiOrchestrator.tsx`, non modifié ici) ; seuls les documents partagés entreront en conflit (résolution « le mien puis `main` »).
* **Contrôle indépendant** : à consigner (revue indépendante, contre-vérification, Green Gate sur la tête exacte, preview).
* **Statut** : `Développé` · `Testé` (banc) · 🟡 **EN PRÉPARATION** — PR brouillon #121 (tête `abddb95` puis renseignement du numéro de PR) ; production contrôlée seulement sur feu vert écrit.
