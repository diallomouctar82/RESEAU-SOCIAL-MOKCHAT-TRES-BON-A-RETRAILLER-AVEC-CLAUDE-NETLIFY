...
* **Module(s)** : Authentification & sessions (`services/auth.ts`, `App.tsx`, nouveau `components/ServeurInjoignable.tsx`, fiche `docs/modules/AUTHENTIFICATION.md` § 5) ; tests `tests/appEntreeReseau.test.tsx`, `tests/sessionValidee.test.ts` ; preuves `docs/captures/2026-09-05-serveur-injoignable-reprise/`.
* **Problème / Besoin initial** : Direction (05/09, après le contrôle cas par cas de DEC-2026-081) : « nous validons le durcissement recommandé. Si le serveur est injoignable et que la session ne peut pas être vérifiée, ne pas ouvrir l'interface. Afficher un message clair de reprise. MokNet est momentanément injoignable, veuillez réessayer. Ne pas effacer la session locale si elle n'a pas été refusée. Dès que le réseau revient, permettre la reprise sans ressaisir inutilement. La page /architecte reste publique uniquement si elle reste démonstration sans données ni fonctions sensibles. Sinon, elle passe derrière le verrou. Le cache des conversations après déconnexion est à traiter dans le chantier messagerie. » Constat (DEC-2026-081, tolérance dite) : une session locale non expirée que le serveur ne pouvait pas confirmer ouvrait l'interface (vide, données inaccessibles) ; une session expirée hors ligne laissait une roue de chargement 25 s puis l'écran de connexion, le jeton conservé par supabase-js n'était plus réutilisé.
* **Idées / Options envisagées** : (1) garder la tolérance — refusée par la Direction (« aucune personne sans session valide ») ; (2) fermer sur l'écran de connexion en effaçant la session — refusée : la session n'a pas été refusée, la Direction exige la reprise sans ressaisie ; (3) **écran de reprise dédié, session conservée, reprise automatique** — retenue ; (4) `/architecte` derrière le verrou — non nécessaire après audit : la page ne lit que la configuration de l'avatar déjà en mémoire (`adminConfigService.getDetailedSettings().architecteAvatar`, valeurs par défaut livrées avec l'application ; la table `platform_settings` n'existe pas dans le projet Supabase, vérifié le 05/09), joue un fichier audio statique et la voix du navigateur, n'écrit rien, n'appelle aucun service payant ni aucune donnée de compte → reste publique, conformément à la condition de la Direction. Constat annexe du contrôle indépendant (C12, préexistant, hors périmètre) : l'enveloppe applicative (`adminConfigService`, singleton construit au chargement du bundle sur toute route, écran de connexion compris) émet après 500 ms une lecture de `profiles` (vide sans session : RLS `profiles_select_visible` réservée à `authenticated`, vérifiée sur le projet) et ouvre un canal Realtime par visiteur anonyme — arbitrage séparé proposé à la Direction : conditionner `initSupabaseRealtimeSync` à une session vérifiée ; `docs/supabase_schema.sql` (« viewable by everyone ») est périmé par rapport à la production.
* **Décision retenue — fermé par défaut avec écran de reprise** :
  - `services/auth.ts#relireSession()` : relecture détaillée de la session gardée par l'appareil — « session », « aucune », ou « injoignable » (jeton expiré que supabase-js n'a pas pu rafraîchir, `AuthRetryableFetchError` : la session reste dans le stockage, non refusée). `getSession()` et `verifierSession()` inchangés.
  - `components/ServeurInjoignable.tsx` : écran de reprise — message exact « MokNet est momentanément injoignable, veuillez réessayer. », bouton « Réessayer » (≥ 44 px, neutralisé pendant la tentative, `aria-busy`), rappel des reprises automatiques, détail technique en petit ; même habillage que l'écran de connexion (`data-miroir`, `mir-sheet`).
  - `App.tsx` : verdict `non-verifiee` → écran de reprise, jamais l'interface, jamais `signOut` ; un verdict non vérifié n'est **jamais mémorisé** au-delà de sa résolution (chaque tentative interroge le serveur ; un jeton refusé reste refusé) ; relecture initiale **plafonnée à 8 s** (`DELAI_VERIFICATION_SESSION_MS`) : au-delà, écran de reprise sans attendre, résultat tardif appliqué dès qu'il arrive ; **une seule tentative à la fois**, partagée par la relecture initiale, le bouton, l'événement `online`, une minuterie de 30 s et une tentative programmée juste après la fenêtre de 60 s pendant laquelle supabase-js garde en cache l'échec de rafraîchissement d'un jeton expiré (`REFRESH_FAILURE_COOLDOWN_MS`, constat C1 de la revue : sans elle, « Réessayer » échouait sans appel réseau jusqu'à 60 s après le retour du réseau) ; au retour sur la page, le rejeu `SIGNED_IN` de supabase-js passe par le même verrou (un second écouteur doublait le chargement du profil, retiré au constat C3) ; l'`INITIAL_SESSION` nul que supabase-js émet quand son rafraîchissement échoue hors ligne ne ferme pas l'écran de reprise ; `SIGNED_OUT`, la déconnexion et un refus du serveur le ferment (écran de connexion).
  - Rien supprimé, aucun autre écran modifié ; sans session sur l'appareil, l'écran de connexion reste l'entrée (serveur joignable ou non).
* **Preuves** : typage 0 ; `tests/appEntreeReseau.test.tsx` 28 cas (16 antérieurs conservés, dont le cas « tolérance dite » révisé en « fermé par défaut », +12 : message et bouton, session non effacée, reprise par bouton / `online` / `visibilitychange` / minuterie 30 s, serveur toujours injoignable, relecture « injoignable », relecture qui traîne au-delà du plafond, `INITIAL_SESSION` nul puis `SIGNED_OUT`, refus à la reprise, `SIGNED_IN` rejoué pendant l'écran, aucune session + serveur injoignable) ; `tests/sessionValidee.test.ts` 16 cas (+4 `relireSession`) ; suite complète 1 658/1 658 (109 fichiers) ; build propre. Banc navigateur (Chromium Playwright, ordinateur 1440 × 900, téléphone Chrome Android 390 × 844, tablette Safari iPad 820 × 1180, Supabase intercepté avec bascule en cours de scénario) sur le build de la branche : **78/78 mesures conformes** (26 scénarios × 3 supports, build corrigé de la tête `93a1aad`, `docs/captures/2026-09-05-serveur-injoignable-reprise/`) — serveur coupé avec session non expirée → écran de reprise ; retour du réseau (`online`), retour sur la page (rejeu `SIGNED_IN` de supabase-js) ou bouton → interface en ≈ 1,1 s, jeton conservé, sans ressaisie ; serveur toujours injoignable → écran maintenu ; refus à la reprise → effacement + connexion ; session expirée hors ligne → écran de reprise en 8,1 s ; réseau revenu pendant les reprises internes de supabase-js → interface en ≈ 4,1 s ; réseau revenu 35 s après l'ouverture (reprises épuisées, cache d'échec de 60 s) → interface ≈ 42 s après le retour du réseau, sans ressaisie ; tous les cas de DEC-2026-081 (connecté, déconnecté, sans compte, navigation privée, session invalide/expirée, liens privés, `/architecte`) inchangés.
* **Contrôle indépendant** : revue indépendante (agent séparé, producteur ≠ contrôleur) sur `9d4d957`→`b4b23d0` : « À CORRIGER », aucun bloquant — C1 IMPORTANT (cache d'échec de 60 s de supabase-js : reprise d'une session expirée non immédiate au retour du réseau), C2 IMPORTANT (preuves P3 absentes de la tête revue), C3–C13 mineurs (double écouteur de visibilité, tentative et relecture non partagées, promesses sans `.catch`, constante entre imports, contraste du détail technique, `role="alert"` englobant le bouton, focus perdu sur `disabled`, texte « 30 secondes » en dur, cas de test manquants, formulation `/architecte`, limite `/token`) ; toutes corrigées sur `93a1aad` (tentative programmée après la fenêtre du cache avec garde-fou sur la constante de auth-js, tentative et relecture uniques, écran accessible, 7 tests ajoutés, documents précisés) ; contre-vérification en cours sur la tête finale.
* **Limites dites** : navigateurs émulés, pas d'appareil physique — le contrôle sur téléphone et tablette réels appartient à la Direction ; pour une session **expirée** pendant la coupure, la reprise n'est pas immédiate au retour du réseau : supabase-js garde 60 s l'échec de rafraîchissement du même jeton (aucune API publique ne contourne ce cache), la tentative programmée juste après cette fenêtre rouvre l'interface au plus tard ≈ 1 min après le retour du réseau, sans ressaisie (dit sur l'écran) ; avec un jeton expiré, une réponse du serveur autre que 5xx/réseau sur `/auth/v1/token` (429, 400 inattendu) fait retirer la session par supabase-js lui-même (C13) ; le résidu `lmav_chat_conversations_cache` après déconnexion relève du chantier messagerie (décision Direction).
* **Statut** : 🟠 **EN PR (#120, brouillon)** — Green Gate, preview Netlify, revue indépendante et contre-vérification en cours ; aucune production sans feu vert écrit de la Direction.
