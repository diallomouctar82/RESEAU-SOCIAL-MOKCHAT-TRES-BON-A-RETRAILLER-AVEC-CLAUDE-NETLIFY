...
### [DEC-2026-085] — 5 Septembre 2026
* **Module(s)** : Diallo OS & Architecte (module 01) — barre flottante `components/architecte/ArchitecteFloatingBar.tsx` ; moteur vocal partagé `services/voiceEngine.ts` (lecture HD, repli navigateur, file d'énoncés) ; tests `tests/voiceEngine.test.ts`, `tests/ArchitecteFloatingBar.test.tsx` ; preuves `docs/captures/2026-09-05-architecte-continuite/`. Numéro réservé : DEC-2026-082 / v6.43.0 sont sur `main` (PR #115), DEC-2026-083 / DEC-2026-084 / v6.43.1 sont pris par la PR #117 en cours — renuméroté si `main` en décide autrement.
* **Problème / Besoin initial** : Direction (05/09, production réelle v6.42.1, téléphone et ordinateur) : « L'avatar est bien là, visible et vivant sur téléphone et ordinateur, mais après sa présentation, il ne porte plus la conversation. Ce n'est pas l'objectif. Il faut que l'avatar accompagne toute la conversation et soit synchronisé aux réponses. Corrige cela dans le même périmètre et envoie une preuve réelle téléphone et ordinateur où on voit l'avatar qui suit toute la conversation. »
* **Audit (causes racines, `file:ligne` sur `main` `f6d9a16`)** : (1) `voiceEngine.attachOutputAnalyser` reliait chaque phrase HD — `new Audio()` par phrase — au graphe d'un `AudioContext` créé hors geste et seulement « résumé » depuis une promesse : resté `suspended` (Safari/iOS), l'élément jouait jusqu'à `ended` sans qu'aucun son sorte ni que l'analyseur ne bouge la bouche — muet et immobile, sans erreur ; (2) `speakWithElevenLabs` : première phrase refusée à `play()` → `break` → `return true` : silence pris pour un succès, aucun repli ; (3) `onClick={isOpen ? close : ouvrirParLaSculpture}` : toucher la sculpture ouverte coupait écoute, voix et session — le geste naturel de qui veut lui parler ; (4) chien de garde du micro (3 s) aveugle au fait que la voix met le micro en pause exprès pendant qu'elle parle → « Le micro n'a pas démarré — utilisez la saisie » et présence `error` au milieu de l'accueil ; (5) `finally { setIsThinking(false) }` dès l'envoi de la réponse, avant le premier son (1 à 4 s de génération) → avatar au repos entre deux ; (6) minuteries de la file navigateur (différé anti-cancel 80 ms, respiration, chien de garde 2 s) sans époque : une minuterie périmée a piloté la file suivante au banc (défaut latent).
* **Idées / Options envisagées** : (a) forcer la lecture automatique — impossible, c'est le navigateur qui décide ; (b) repli systématique sur la voix du navigateur sur téléphone — refusé (identité vocale attitrée, § 9 Équipe B) ; (c) contexte audio créé dans le geste et élément unique réutilisé (technique éprouvée des lecteurs mobiles), vérification du contexte avant de relier — **retenu** ; (d) garder « la sculpture referme » et ajouter un bouton « parler » — refusé (un bouton de plus, alors que le ✕ existe déjà et que le geste sur l'avatar EST la demande de parole) ; (e) présentation rejouée une fois par appareil au lieu d'une fois par session de page — non touché (décision validée par la Direction, hors du défaut constaté).
* **Décision retenue — continuité de l'avatar sur toute la conversation, même périmètre, rien retiré** :
  - `voiceEngine.unlockPlayback()` (public, idempotent, jamais bloquant) appelé DANS les gestes de la barre (appui sur la sculpture fermée ou ouverte, bouton Présentation, envoi clavier) et au mieux à `open()` : démarre le contexte de sortie, crée UN `HTMLAudioElement` (`playsinline`, `preload=auto`) qui joue un clip WAV silencieux construit en mémoire (`SILENT_WAV_DATA_URI`, 8 échantillons) — l'élément reste autorisé pour toute la page et porte ensuite chaque phrase HD ; `isPlaybackUnlocked()` pour le dire honnêtement.
  - `playAudioUrl` : `ensureOutputContextRunning()` (attente bornée à 150 ms, `null` si personne n'écoute le niveau ou si le contexte ne démarre pas), `pickPlaybackElement()` (l'élément déverrouillé, sauf s'il est relié à un contexte qui ne tourne pas — alors un élément libre en direct), `readyTracks` purgé par phrase, piste phonétique gardée par URL ; `attachOutputAnalyser` ne relie l'élément au graphe QUE si le contexte est `running` ; la boucle de bouche tourne aussi sans analyseur (piste seule) et l'horloge repart à chaque clip.
  - `speakWithElevenLabs` : première phrase refusée → `finishSpeakingElevenLabs(options, 'relais')` (ni `onEnd`, ni micro relancé) et `return false` → repli navigateur, qui ne répète pas `onStart` déjà émis par le HD (`relayOptions`) ; **sans synthèse navigateur** (ou rien à dire), `abandonSpeech` dit `SPEECH_OUTPUT_FAILED_MESSAGE`, appelle `onEnd` UNE fois et relance l'écoute conversationnelle — jamais un tour laissé ouvert pour les autres écrans (LIVE, coachs, réglages voix). Le flux de capture micro est **conservé** pendant la lecture (une première version le relâchait ; retiré à la revue : bascule de route audio et indicateur micro à chaque tour, sans preuve du gain).
  - **iPhone / iPad : lecture directe** (`prefersDirectPlayback`) — la voix n'est jamais reliée au graphe Web Audio sur ces appareils, où la sortie Web Audio suit l'interrupteur silencieux alors qu'un élément média s'entend ; la bouche suit la piste phonétique (décodée à part). Refus de démarrage du contexte mémorisé jusqu'au prochain geste (pas 150 ms d'attente par phrase) ; horloge de la bouche remise par lecture (`playbackSerial`), latence de sortie relue à chaque image ; déverrouillage idempotent pendant le clip silencieux.
  - File navigateur : époque `speakEpoch` capturée par `fallbackToBrowserSpeech` et vérifiée par le différé 80 ms, la respiration, le chien de garde et `processNextInQueue(options, epoch)`.
  - Barre : `reprendreLaParole` sur la sculpture ouverte (déverrouille, coupe la présentation si elle joue, se tait s'il parlait, « Je vous écoute. », relance l'écoute si besoin), libellé « Parler à l'Architecte » ; fermer reste sur le ✕ « Fermer » ; `armListenWatchdog` patiente tant que `speakingRef` est vrai ; `dire()` porte la présence (jeton `speechTokenRef`, `beginThinking` / `settleThinking`, `onStart` de la voix relâche la réflexion) sur tous les chemins qui réfléchissent (commande, vision, recherche web, fichier, document).
* **Preuves** : `tests/voiceEngine.test.ts` (+9 : déverrouillage idempotent, élément réutilisé relié une fois, contexte suspendu → non relié, relais → repli et `onEnd` unique, relais sans synthèse → `onEnd` + message honnête + micro relancé, `onStart` unique, iPhone → jamais relié, deux gestes → un clip, refus mémorisé), `tests/ArchitecteFloatingBar.test.tsx` (+4, doublure du hook étendue : `isSpeaking`, options de `speak`, `unlockPlayback` espionné) ; `npx tsc --noEmit` = 0 ; **1651/1651 tests (109 fichiers)** ; `npm run build` ✓. Banc Chromium avant/après (`docs/captures/2026-09-05-architecte-continuite/`, README, mesures JSON par écran, script `preuve-c1.cjs`) : ordinateur 1440 × 900 et téléphone 390 × 844, voix HD doublée par un extrait réel de la voix attitrée, reconnaissance vocale factice pilotée par le banc — après : 3 paroles HD, moteur `elevenlabs` seul, piste phonétique alignée à chaque phrase, bouche ouverte sur 71 %/72 % des images pendant la parole et 0 hors parole, 1 liaison au graphe (5 avant), appui pendant la réponse → barre ouverte, « Je vous écoute. », présence `listening`, écoute repartie 3 fois (2 avant) ; avant : appui → barre fermée, présence `rest`, mode conversationnel coupé.
* **Contrôle indépendant** (agent séparé, producteur ≠ contrôleur, 05/09) : contre-vérification « PRÊT » sur `0604be6` (delta `50f481c..0604be6`, 7 contre-épreuves du contrôleur, suite 1655/1655) avec quatre mineurs à faible coût traités dans la foulée (drapeau de déverrouillage libéré sur exception, `onEnd` quand il n'y a rien à dire, ligne vide en fin de fichier, remise à zéro complète du banc) — contre-vérification courte « PRÊT » sur `ce59052` (93/93 tests ciblés, `git diff --check` propre, 0 retrait d'une autre équipe ; suite complète 1656/1656 sur 109 fichiers et build ✓ sur cette tête) ; verdict initial « À CORRIGER » sur `50f481c` — 2 IMPORTANT (relais HD → repli sans synthèse navigateur : `onEnd` jamais appelé, aucun message — régression du contrat pour LIVE, coachs et réglages voix ; `main` avancé pendant la revue) et 9 MINEUR (`onStart` émis deux fois sur le relais ; `reprendreLaParole` ne coupait pas une réponse encore en génération ; déverrouillage non idempotent pendant le clip ; horloge non remise pour deux phrases de même URL ; latence figée au démarrage de la boucle ; 150 ms d'attente par phrase quand le contexte refuse ; assertion « bienvenue » affaiblie ; commentaire d'en-tête périmé ; changement de rôle de la sculpture ouverte à nommer à la Direction) — **tous traités** dans `c396623` (le retrait du relâchement du flux micro répond au risque (b) nommé par le contrôleur).
* **Limites dites** : le mutisme par contexte suspendu (Safari/iOS) n'est pas reproductible en Chromium headless (activation collante dès le premier clic) — couvert par les tests du moteur ; sur iPhone la sortie Web Audio suit l'interrupteur silencieux : c'est pourquoi la voix y sort en direct de l'élément (non vérifiable au banc, à contrôler sur appareil réel par la Direction) ; l'ouverture par le dock (`openSignal`) déverrouille hors du gestionnaire du geste (au mieux : accordé sur Chrome, non prouvé sur Safari — le premier appui sur la sculpture ou l'envoi d'un message le fait dans le geste) ; la hausse du taux d'ouverture de la bouche au banc (40 % → 71 %) n'est pas expliquée et n'est pas présentée comme un effet de C1 ; sur iPhone/iPad en lecture directe, la bouche dépend entièrement de la piste phonétique (plus de repli « amplitude mesurée ») : si l'alignement échoue, la voix s'entend bouche fermée (`data-lipsync` le dit) ; « réfléchit » trop bref pour être échantillonné avec une passerelle instantanée (prouvé par test DOM) ; la justesse phonème par phonème n'est pas l'objet (extrait vocal identique pour chaque phrase) ; le contrôle sur téléphone réel appartient à la Direction.
* **Feu vert** : aucun pour l'instant — « rien n'est fusionné sans feu vert ». La sculpture OUVERTE change de rôle (elle ne referme plus : « Parler à l'Architecte » ; fermer = ✕) : interprétation du producteur du constat de la Direction, nommée ici et dans la PR pour son contrôle visuel.
* **Statut** : 🟠 **PRÊTE POUR REVUE INDÉPENDANTE** (code, tests, preuves de banc) — production contrôlée uniquement sur feu vert écrit de la Direction.
...
* **Contrôle cas par cas après production (Direction, 05/09 : « aucune personne sans compte ou sans session valide ne doit accéder à l'application », téléphone, ordinateur, tablette, navigation privée, liens WhatsApp/Messenger/SMS, liens privés)** — bundle réellement servi par `moknet.net` à 18:26 UTC (`index-C8Y6seFM.js`, 5 435 227 octets, même empreinte SHA-256 que le miroir, quatre marqueurs du verrou présents, ancien bundle → 404), Chromium Playwright émulant ordinateur 1440 × 900, téléphone Chrome Android 390 × 844 et tablette Safari iPad 820 × 1180, Supabase intercepté par état : **54/54 mesures conformes** (`docs/captures/2026-09-05-acces-public-authentification/cas-par-cas/`, 22 planches, mesures JSON, script). Connecté avec session confirmée → interface (y compris par lien privé `?live=`) ; compte mais déconnecté (bouton « Se déconnecter » du tiroir téléphone / du menu de profil ordinateur-tablette) → connexion immédiate, jeton et profil local effacés, réouverture → connexion ; session refusée, expirée, refusée puis rejouée `SIGNED_IN`, refusée par lien privé → effacement + connexion ; sans compte / navigation privée (stockage vide) → connexion avec onglet « Créer un compte » ; liens privés sans session (`?live=`, `?invite=`, `/messagerie`, `?module=messagerie`, `#admin`) → connexion ; serveur injoignable sans session → connexion ; serveur injoignable avec session **expirée** → roue de chargement 25,5 s (huit rafraîchissements tentés par supabase-js) puis connexion, jamais l'interface ; serveur injoignable avec session **non expirée** → interface (tolérance dite, bornée par la durée de vie du jeton, données inaccessibles) — durcissement proposé à la Direction, non appliqué sans décision. Exception tracée : `/architecte` s'affiche sans session (page publique de démonstration DEC-2026-066, aucune donnée de compte) — à confirmer ou fermer par la Direction. Résidu dit après déconnexion : `lmav_chat_conversations_cache` et `lmav_enrolled_persons_v1` restent sur l'appareil (aucun accès n'en découle ; hors périmètre). Niveau de preuve : 🌐 bundle de production servi, navigateurs émulés — le contrôle sur téléphone et tablette physiques appartient à la Direction (protocole dans le README). Outillage : `scripts/production-controlee/mirror-serve.py` sert désormais les chemins sans extension en `text/html` (réécriture SPA), sinon Chromium ne rendait pas `/messagerie` ni `/architecte`.
...
* **Ajustement v6.42.1 (5/09/2026, point orange traité sur instruction de la Direction : « tu appliques le remède annoncé, tu montres une preuve avant-après sur mobile et ordi, puis seulement après tu mets en production contrôlée »)** : la bande sombre au sommet du cadre rond de `/architecte` venait du débord du cadre (photo sans marge au-dessus du crâne) laissé transparent par `drawFramed` puis rendu noir par l'export JPEG — 95 px sur 768 dans le portrait, 89 px sur 720 dans le modèle HeyGen, 44 px sur 360 dans l'affiche. **Remède dans le moteur de production** : débord comblé par un prolongement adouci du fond (miroir des bords, flou par sous-échantillonnage 32 px, fondu vers la teinte du bord ; canvas 2D portable), masque de silhouette inchangé dans son principe (débord transparent), **avertissement** nommant le côté dès 2 % (`overflowBands` / `describeOverflow`, testés) ; portrait d'usine et masque régénérés par ce moteur dans Chromium réel (cadrage identique, calage d'usine conservé, ± 0,2 point de bruit sur mâchoire / menton / bouche) ; vidéo plein cadre recomposée sur le portrait corrigé à travers la silhouette du détourage (**aucun crédit HeyGen**), affiche régénérée, registre des séquences à jour. **Revue indépendante avant fusion** (contrôleur séparé, huit constats, tous traités) : filet sombre résiduel de 4 px dans la vidéo et l'affiche corrigé (rampe de fondu reportée après la bande : poids 1 jusqu'à la ligne 92, 0 à 112, écart ligne à ligne < 1,5 entre 10 et 14 %), socle du prolongement, géométrie des tuiles testée, empreintes du portrait / silhouette / affiche signées par un test, captures vidéo avec le sommet du cadre à l'écran, numéro v6.42.1 (v6.42.0 prise par `main` pendant le contrôle). Preuves sur l'arbre livré : typage 0, 1642/1642 tests (109 fichiers), build ✓, `docs/captures/2026-09-05-architecte-bande-sombre/` (luminance de la bande 0 → 131–136 pour le portrait et 0 → 132 pour la vidéo, sur trois écrans, au repos et vidéo en lecture). Statut de l'ajustement : 🟢 EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 17:48 UTC — PR #111 fusionnée en squash → `main` `f6d9a16` à 17:47 UTC après Green Gate vert sur `d5550ed` (run 33981843750), `main` vérifié inchangé (`e9595e5`) à 17:46:58 UTC et contre-vérification indépendante « PRÊT » ; `moknet.net` sert `index-C8Y6seFM.js` (marqueurs du remède, nouvelles empreintes, anciennes absentes, actifs corrigés aux tailles exactes, ancien bundle → 404) ; fumée Chromium sur la page servie : bande à 132 pour le portrait et la vidéo sur ordinateur et téléphone ; Green Gate sur `main` run 33982039577.
...
* **Idées / Options envisagées** : (1) calage et détourage faits hors ligne par l'organisation à chaque photo (rembg + mesures, méthode du playbook 16) — refusée comme seule voie : la Direction veut que l'avatar prenne **automatiquement** la forme de la photo fournie ; (2) repères et segmentation dans le navigateur avec MediaPipe Tasks Vision (Face Landmarker, 478 repères dont les iris ; Selfie Segmenter) — **retenue** : même technique que l'avatar validé (photo cadrée comme le portrait d'usine, rig 2D, masque de silhouette), technologie libre (Apache 2.0), aucune photo envoyée à un tiers ; (3) régénération du modèle vidéo HeyGen depuis la photo — conservée pour la suite (563 crédits disponibles), après réception du fichier et validation par la Direction.
* **Décision retenue — la photo devient l'avatar vivant, automatiquement, dans la carte** :
  - `services/architecte/photoAvatar.ts` (domaine pur) : cadrage carré reproduisant le portrait d'usine (pupilles à 46,3 % de la hauteur, écart 21,5 % de la largeur — `FRAMING_TARGET`), `rigFromLandmarks` (pupilles, paupières, sourcils, ligne entre les lèvres, menton, coins de la bouche → `PortraitRig` + `MouthAnchor`, avertissements non bloquants), `featherAlpha` (masque adouci), `applyPhotoAvatar` / `revertPhotoAvatar` / `snapshotOf` (une seule version précédente conservée).
  - `services/architecte/photoAvatarEngine.ts` (navigateur) : décodage, analyse ≤ 1024 px, Face Landmarker, découpe 768 px, segmentation 512 px lue dans le bon sens (`orientPersonMask`), débord transparent, photo JPEG + masque PNG en data URL ; wasm `@mediapipe/tasks-vision` 1.0.1 chargé à la demande depuis jsDelivr, modèles servis avec l'application (`public/architecte/modeles/`, empreintes testées) ; erreurs nommées (lecture, aucun visage, moteur) — sans réseau vers le CDN, le calage manuel reste possible.
  - `ArchitecteAvatarConfig` : `silhouetteMaskForPhotoUrl` (le masque ne vaut que pour SA photo), `previousAvatar` ; `sculptureMaskFor` généralisé ; `ARCHITECTE_PRESENTATION.portraitUrl` + `sequenceFitsPhoto` : le modèle vidéo validé ne se joue que sur le portrait dont il vient — avec une autre photo, l'Architecte parle par le portrait vivant (voix HD), sans vidéo, bouton Présentation absent.
  - Carte Super-Admin : bloc « Créer ou remplacer l'avatar vivant depuis une photo », DANS la carte (aucun panneau par-dessus l'application) : choisir une photo → analyse → aperçu vivant « Actuel / Nouveau » (composant réel en variante sculpture, curseur de voix) + avertissements + note sur le modèle vidéo → « Valider et enregistrer » / « Annuler » → « Revenir à l'avatar précédent ».
  - Rien supprimé : adresse de photo, calage manuel, animations, synchro labiale, séquences, remise par défaut inchangés.
* **Preuves** : `tests/photoAvatar.test.ts` (11 : cadrage, pupilles, rig mesuré sur un visage synthétique, avertissements, masque, validation / retour, masque par photo, orientation de la confiance, empreintes des modèles), `tests/adminArchitecteAvatarPhoto.test.tsx` (4 : dans la carte sans overlay, aperçu, validation + retour arrière, annulation, erreur puis réessai) ; suite complète 1 588 tests / 104 fichiers ✓ ; typage 0 ; build ✓ (bundle principal +13 240 octets, moteur MediaPipe en morceau séparé de 154 977 octets chargé à la demande, modèles 4,0 Mo servis avec l'application).
* **Essai navigateur réel (🗄️ banc `design-lab/banc/super-admin-avatar.html`, Chromium 1194, moteur MediaPipe RÉEL — wasm servi localement à la place du CDN, modèles servis par l'application —, photo d'essai = portrait d'usine inversé et teinté faute de la photo de la Direction, ordinateur 1280 × 800 et téléphone 390 × 844, 14 captures `docs/captures/2026-09-05-architecte-avatar-depuis-photo/`, mesures `mesures.json`)** : analyse en 1,9 à 2,5 s, 478 repères ; rig mesuré (yeux 46,3 %, pupilles 39,3 / 60,7 %, lèvres 69 %, menton 83 %, sourcils 38,4 %), bouche (49,7 / 69 %, largeur 23,2 %, inclinaison 1,5°) ; masque 512² : 136 048 pixels opaques, 123 902 transparents, 2 194 de bord adouci, centre du visage opaque, coins transparents ; photo enregistrée 90 371 octets, masque 25 714 octets ; aucun élément fixe hors de l'ancrage de l'Architecte sur la page Super-Admin ; application avec le nouvel avatar : sculpture détourée par son propre masque, obstruction 0,9 % / 2,1 % au repos et 2,5 % / 6,4 % barre ouverte, aucun panneau, bouton Présentation absent (modèle d'un autre portrait) ; retour arrière : portrait et masque d'usine rétablis, plus rien à quoi revenir ; 0 erreur (seuls les messages INFO de TensorFlow Lite).
* **Limites dites** : la photo validée n'a pas encore été traitée (fichier non reçu — ACTION REQUISE : la déposer via l'option Super-Admin une fois en production, ou dans Google Drive) ; le modèle vidéo HeyGen sur la nouvelle photo reste à générer puis à faire valider (563 crédits disponibles, aucune dépense sans accord) ; la voix reste celle réglée dans la carte (à harmoniser avec le nouveau visage sur décision de la Direction) ; la photo et son masque vivent en data URL dans les réglages Admin (≈ 120 Ko) — un stockage dans le bucket public est une amélioration possible ; Safari/iOS non essayés ; le détourage vient du Selfie Segmenter (bord adouci), pas d'un relevé image par image comme pour la vidéo validée.
* **Statut** : 🟢 **EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 14:25 UTC** — PR #101 fusionnée en squash → `main` `f64f7ee` (Green Gate vert sur `5bb9566`, run 33971591543) après contrôle de coordination (`main` inchangé sur 27d6d61 dans la minute de la fusion, aucune autre fusion en cours) ; `moknet.net` sert `index-Bm6woHcd.js` depuis 14:25:08 UTC (5 423 096 octets) — marqueurs de l'option présents dans le bundle servi (« Créer ou remplacer l'avatar vivant depuis une photo », `avatar-photo-choisir`, `avatar-photo-valider`, « Revenir à l'avatar précédent », `silhouetteMaskForPhotoUrl`, chemin du modèle), morceau MediaPipe `vision_bundle-BP1Ar9Ko.js` servi (HTTP 200, 154 977 octets), modèles servis avec leurs tailles exactes (`face_landmarker.task` 3 758 596 octets, `selfie_segmenter.tflite` 249 537 octets), ancien bundle `index-D0ScDC87.js` → 404, `cache-control: public,max-age=0,must-revalidate`, `/architecte` → 200. Non vérifié en production faute de compte Super-Admin de banc : l'analyse d'une photo réelle sur `moknet.net` (moteur et modèles servis, prouvés au banc) — à confirmer par la Direction en déposant sa photo.

### [DEC-2026-076] — 5 Septembre 2026
* **Module(s)** : Diallo OS & Architecte (module 01) ; entrée de l'application (`App.tsx`).
* **Problème / Besoin initial** : correction finale de la Direction (05/09) : « L'identité officielle reste "je suis l'Architecte de Vision Smart". Supprime toutes les phrases du type je suis l'avatar ou je suis une IA. Une seule voix harmonisée, ton simple et bref. S'il ne comprend pas, il le dit. S'il doit présenter Vision Smart, il donne la description validée. Ensuite, plus de répétition d'identité à chaque réponse. Corrige aussi la redirection : dès la première ouverture, connexion ou inscription, entrée directe sur Réseau MokNet, sans rechargement. » **Constaté** : la réponse d'identité déterministe et l'accueil disaient « Je suis L'Architecte — votre guide dans Le Monde à Vous » ; le prompt n'interdisait pas explicitement « je suis une IA / un avatar » ni la répétition d'identité ; aucune description validée n'était portée ; après une déconnexion, l'onglet retombait sur le tableau de bord (`'home'`), donc la connexion suivante n'entrait pas sur Réseau MokNet ; un retour d'historique sans état retombait aussi sur `'home'`.
* **Idées / Options envisagées** : (1) laisser le modèle formuler l'identité — refusée (dérive possible vers « je suis une IA ») ; (2) **retenue** : identité et présentation dans des constantes déterministes, règles explicites dans le prompt, entrée sur Réseau MokNet fixée dans l'application. Description validée : aucune description de Vision Smart validée n'existe dans les dépôts ; retenu : le script du modèle validé par la Direction, porté à l'identité officielle, dans une seule constante `VISION_SMART_PRESENTATION` (à remplacer si la Direction fixe un autre texte officiel — une ligne).
* **Décision retenue** :
  - `services/architecte/architecteBrain.ts` : `ARCHITECTE_IDENTITY_SENTENCE` = « Je suis l'Architecte de Vision Smart. » ; `VISION_SMART_PRESENTATION` (« Je suis l'Architecte de Vision Smart. Je suis là pour accompagner, expliquer et guider, avec une voix claire, naturelle et professionnelle. ») ; `isVisionSmartPresentationRequest` (« présente Vision Smart », « c'est quoi Vision Smart ? »…) traité **sans modèle** ; `describeArchitecteIdentity` brève ; accueil de première rencontre réécrit ; prompt : identité officielle, interdiction de « je suis une IA », « je suis un avatar », « je suis un assistant virtuel », « en tant qu'intelligence artificielle », pas de répétition d'identité, ton simple et bref, « Je n'ai pas compris » + une question courte quand la demande n'est pas comprise ; repli d'exécution : « Je n'ai pas compris ce que je dois faire. Reformulez… ».
  - Barre flottante : statut d'échec vidéo « L'Architecte reste avec vous » (plus « l'avatar animé »).
  - `App.tsx` : après déconnexion et sur un retour d'historique sans état, l'onglet est Réseau MokNet (`'social'`) — la connexion ou l'inscription suivante y entre directement, sans passer par le tableau de bord et sans rechargement (aucun `location.reload/assign/href` sur le chemin ouverture → connexion → entrée).
  - Rien d'autre touché.
* **Preuves** : `tests/architecteSession.test.ts` (identité brève sans « avatar » ni « IA », description validée servie sans modèle, prompt : interdictions, non-répétition, ton bref, « Je n'ai pas compris ») ; `tests/appEntreeReseau.test.tsx` (App réelle avec doublures d'écrans : première connexion → Réseau MokNet, déconnexion puis reconnexion → Réseau MokNet, retour d'historique → Réseau MokNet, garde-fou « aucun rechargement » sur `App.tsx`, `components/Auth.tsx`, `services/auth.ts`) ; suite complète 1 573 tests / 102 fichiers ✓ ; typage 0.
* **Limites dites** : l'audio du **modèle vidéo validé** prononce encore « Bonjour, je suis l'avatar de Vision Smart » (phrase enregistrée dans le fichier validé ; les sous-titres restent fidèles à l'audio) — décision Direction : régénérer le modèle avec la phrase officielle (HeyGen, crédits existants, nouvelle validation) ou le conserver ; inscription avec confirmation d'e-mail : le lien de confirmation ouvre l'application (nouvelle page par nature), puis l'entrée est directe ; connexion Google : aller-retour OAuth par nature ; l'entrée réelle après connexion sur `moknet.net` n'est pas vérifiable sans compte (contrôle visuel de la Direction).
* **Statut** : 🟢 **EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 14:11 UTC** — même PR (#98), même squash `022a224` et même bundle `index-D0ScDC87.js` que DEC-2026-075 (marqueurs d'identité et d'accueil vérifiés dans le bundle servi).

### [DEC-2026-075] — 5 Septembre 2026
* **Module(s)** : Diallo OS & Architecte (module 01) ; banc de preuve `design-lab/banc/sculpture.html`.
* **Problème / Besoin initial** : consigne ferme de la Direction (05/09, après la mise en production de la v6.37.0) : « Il n'est pas question que l'avatar arrive à masquer l'écran de fond, le fond d'écran et les options de MokNet. L'avatar est un assistant… il doit juste guider dans l'interface que l'utilisateur voit », puis : « Tu ne masques jamais l'écran de fond ni les options de MokNet. L'avatar est un assistant, pas l'interface principale. Aucun panneau, aucun fond ne doit couvrir l'application. Au clic, une petite zone de communication au maximum, repliable, sans jamais cacher MokNet. Photo de preuve obligatoire. » **Cause racine reproduite** : la règle « texte quand il apporte une valeur » (§16-17 de la barre) faisait s'ouvrir le panneau **tout seul** dès qu'une réponse dépassait 220 caractères, ou dès qu'une image, un document ou une vidéo entrait dans la session (`isPanelOpen ?? (hasRichTurns || lastIsWrittenProduction || mediaView !== null)` — la flèche « à trois états » de la v6.37.0). L'accueil de première rencontre fait 219 caractères (juste sous le seuil), mais toute vraie réponse du modèle est plus longue : au premier échange réel, le panneau (jusqu'à 22 rem × 40 vh) apparaissait par-dessus MokNet sans que la personne l'ait demandé.
* **Idées / Options envisagées** : (1) garder l'ouverture automatique avec un panneau plus petit — refusée : c'est encore une ouverture que la personne n'a pas demandée ; (2) supprimer le panneau — refusée : la Direction veut une zone repliable pour lire quand on en a besoin, et rien ne doit être supprimé ; (3) **retenue** : le panneau ne s'ouvre JAMAIS de lui-même, un contenu à voir est signalé, et seule la personne déplie.
* **Décision retenue — l'Architecte guide dans l'interface visible, jamais par-dessus** :
  - `isPanelOpen` devient un booléen, faux par défaut et remis à faux à la fermeture (avec la saisie) : rouvrir repart toujours de la barre minimale ; `showConversationPanel = isPanelOpen` — plus aucune condition automatique.
  - **Contenu à voir, signalé** : une réponse écrite longue, une image, un document ou une vidéo en attente allument un point cyan qui respire sur la flèche (`architecte-panneau-invitation`, libellé « Déplier la conversation — un contenu vous attend ») ; rien ne s'ouvre.
  - **Écrire = une ligne** : le bouton Écrire ouvre une seule ligne de saisie à côté de la barre (`architecte-saisie-ligne`, même largeur bornée), pas un panneau ; le même formulaire passe dans le panneau quand la personne le déplie (le focus suit).
  - **Demande explicite de la personne** (« mets-moi une vidéo », « montre-moi le document ») : le panneau se déplie pour elle ; « ferme la vidéo » ou la croix ramènent à la barre minimale.
  - **Hauteur bornée** : panneau ≤ min(36 vh, 18 rem), fil ≤ 10 rem ; la largeur reste ≤ min(22 rem, 100 vw − 8,5 rem).
  - **Rien supprimé** : présentation validée dans la sculpture, fichier, caméra, voix, vidéo, aperçu de document, fil complet — tout reste accessible, à la demande.
* **Preuves** : `tests/ArchitecteFloatingBar.test.tsx` (36 — 5 nouveaux : accueil complet dit sans panneau, réponse longue signalée jamais imposée, Écrire = une ligne puis panneau autour de la même saisie, fermer/rouvrir repart replié, vidéo demandée dépliée puis refermée ; 3 adaptés : image en session, lettre et liens cliquables passent par la flèche) ; suite complète 1 567 tests / 101 fichiers ✓ ; typage 0.
* **Essai navigateur réel (🗄️ banc `design-lab/banc/sculpture.html` servi par vite, composant réel `ArchitecteFloatingBar` posé sur une page qui ressemble à MokNet, Chromium 1194 réel, ordinateur 1280 × 800 et téléphone 390 × 844 — 16 captures `docs/captures/2026-09-05-architecte-zero-obstruction/`, mesures `mesures.json`)** : part de l'écran couverte par l'interface de l'Architecte (ordinateur / téléphone) — repos 0,9 % / 2,1 % ; présentation en cours 2,4 % / 6,8 % ; après la présentation, accueil dit 2,6 % / 6,8 %, **panneau absent** ; réponse longue injectée (324 caractères, sans modèle) 2,6 % / 6,8 %, **panneau absent, point d'invitation présent** ; Écrire 3,9 % / 9,7 %, panneau absent ; panneau déplié par la flèche 9,7 % / 22,7 % (hauteur entièrement libre au-dessus : 60,6 % / 54,1 %) ; replié 3,9 % / 9,7 % ; fermé 1,0 % / 2,3 %. Sur téléphone, le dock de navigation reste libre et cliquable à chaque étape ; les cartes de contenu restent atteignables (5 sur 6 au repos comme en v6.37.0 — la sculpture recouvre le coin d'une carte —, 4 sur 6 seulement quand la personne a déplié le panneau elle-même). Aucune erreur de console.
* **Limites dites** : l'état après connexion en production n'est pas vérifiable sans compte (le banc joue le composant réel ; contrôle visuel de la Direction demandé) ; Safari/iOS non essayés ; le panneau déplié couvre 22,7 % d'un téléphone — par le choix de la personne, repliable d'un appui.
* **Statut** : 🟢 **EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 14:11 UTC** — PR #98 fusionnée en squash → `main` `022a224` (Green Gate vert sur `4774573`, run 33970158515) après contrôle de coordination (aucune autre fusion en cours, `main` inchangé sur 43b3871 dans la minute de la fusion) ; `moknet.net` sert `index-D0ScDC87.js` depuis 14:11:59 UTC — huit marqueurs présents dans le bundle servi (ligne de saisie, invitation « un contenu vous attend », « Je suis l'Architecte de Vision Smart. », accueil officiel, « Je n'ai pas compris ce que je dois faire », « L'Architecte reste avec vous », `architecte-panneau-invitation`, `min(36vh,18rem)`), ancien bundle `index-DnzvQMzK.js` → 404, `cache-control: public,max-age=0,must-revalidate`, page `/architecte` → 200. Non vérifié en production faute de compte : l'état après connexion (contrôle visuel de la Direction).
...
* **Idées / Options envisagées** : (1) transparence vidéo native (WebM VP9 alpha, HEVC alpha) — refusée : deux encodages, aucun pour tous les navigateurs, rien pour Safari sans outil Apple ; (2) masque statique de silhouette appliqué à la vidéo — essayé et mesuré : la tête bouge de ±19 px (sur 720) pendant la phrase, un masque unique laisse une frange claire du fond de studio autour des cheveux (planche de contrôle) ; (3) segmentation à l'exécution dans le navigateur (modèle ML) — refusée : dépendance lourde, batterie ; (4) service tiers de suppression de fond (connecteur déjà présent) — refusé par la politique d'exécution (action bloquée), et inutile : (5) **détourage image par image en local, hors ligne** avec un modèle de segmentation open source (rembg, `isnet-general-use`, 0,9 s par image sur ce poste), sans aucun achat ni crédit, puis vidéo **empilée** (couleurs en haut, matte en bas) recomposée par un canevas à l'exécution — retenue : le modèle validé n'est pas re-généré (mêmes images, même son copié tel quel), la transparence est exacte à chaque image, et le chemin est le même sur Chromium, WebKit et Gecko.
* **Décision retenue — la sculpture vivante remplace le bouton, le modèle validé parle dedans** :
  - **Actifs détourés** (`public/architecte/`) : `vision-smart-heygen.cutout.mp4` (H.264 High 4.1, 720 × 1440, AAC copié du fichier validé, 2 090 415 octets) et `.cutout.webm` (VP9 + Opus, 1 055 974 octets) — 204 mattes relevées sur les 204 images du MP4 validé ; `architecte-silhouette.png` (masque alpha 512², relevé sur le portrait livré) pour la couche du rig 2D. Empreintes SHA-256 et tailles enregistrées dans le registre et **vérifiées par les tests**.
  - **Calage vidéo ↔ portrait** (`SequenceAlignment`) : HeyGen re-cadre légèrement le portrait qu'on lui donne ; mesuré sur les centroïdes des iris (portrait : pupilles à 51,74 % / 46,17 %, écart 19,89 % ; vidéo image 1 : 51,95 % / 46,84 %, écart 20,90 %) → échelle 0,9515 autour des pupilles, décalage −0,21 % / −0,67 %, appliqué à la couche vidéo de la sculpture seulement : le portrait vivant et la vidéo se succèdent sans saut.
  - **Variante `sculpture` de `ArchitecteAvatar`** : sans cadre rond ni fond ; le portrait vivant (rig 2D, repli technique) porte le masque de silhouette (CSS `mask-image`) ; la vidéo validée est détourée image par image par `ArchitecteSequenceCutout` (vidéo empilée cachée, canevas recomposé à chaque image — `requestVideoFrameCallback` quand il existe, sinon `requestAnimationFrame` ; 224 px de côté au plus, sur le processeur) ; l'état se lit dans une lueur portée par la silhouette (`drop-shadow` de la couleur de la grammaire d'états) ; le masque ne vaut que pour le portrait d'usine (`sculptureMaskFor`) — une autre photo garde le cadre rond.
  - **Barre flottante** : la sculpture (112 px sur ordinateur, 96 px sur téléphone) est le bouton permanent, au même emplacement que l'ancien (`bottom-24 right-3`, `md:bottom-6 sm:right-6`) ; fermée, rien d'autre n'est affiché ; **au clic** la présentation validée est demandée dans le geste (cadre `sculpture`), la barre s'ouvre ancrée à droite et finit sous la silhouette (téléphone : la sculpture monte au-dessus de la barre) ; **l'accueil parlé et le micro attendent la fin de la vidéo** (sinon deux voix, et le micro transcrirait la voix de l'Architecte comme une demande) ; une vidéo qui ne démarre pas en 3 s rend la main (accueil, écoute) ; la présentation joue une fois par session de page, le bouton « Présentation » la rejoue dans la sculpture (la grande fenêtre est retirée, à la demande de la Direction) ; **flèche** « Déplier / Replier la conversation » (`isPanelOpen` à trois états : automatique §16-17, ouvert, replié — la saisie clavier montre toujours son champ) ; la sculpture ouverte referme la barre ; fichier, écrire, caméra, présentation, fermer : toutes les fonctions et connecteurs existants inchangés.
  - **Relecture indépendante intégrée (lecteur)** : jeton par demande de lecture et rejet `AbortError` ignoré (un double appui ne bascule plus en « échec » avec le son qui continue) ; `pause` émis avant `ended` par les navigateurs → l'état `ended` existe réellement (`video.ended` lu) ; `stop(slot)` par cadre ; message d'échec sans fausse promesse ; bouton `/architecte` gouverné par `videoSequencesEnabled` ; `presentationPlaying` filtré sur le cadre ; test `/Répéter en boucle/` rétabli ; date « 5 septembre 2026 » et expressivité en français dans la carte Super-Admin ; classe `relative` retirée quand l'appelant positionne l'avatar (`fixed`) — défaut latent : `relative`, défini après `fixed` dans la feuille Tailwind, l'emportait.
  - **Correction obligatoire « zéro obstruction » (Direction, même jour, sur les premières captures)** : « le fond et toutes les fonctionnalités de l'application doivent toujours rester visibles… au clic, il n'ouvre qu'une petite barre minimale… cette barre ne doit jamais obstruer la navigation ». Livré : un **ancrage unique** en bas à droite (`fixed`, `pointer-events-none` — l'application reste cliquable sous lui) qui porte la sculpture et, ouverte, une **colonne étroite à sa gauche** : petite barre d'icônes seules (présentation, fichier, écrire, caméra, flèche, fermer — libellés en info-bulle et pour les lecteurs d'écran), légende d'état (« L'Architecte », point d'écoute, égaliseur minimal, statut tronqué), panneau **étroit** (≤ 22 rem × 40 vh) seulement à la demande ou quand la parole demande quelque chose à voir ; plus rien de centré ni en pleine largeur ; sculpture 96 / 84 px, lueur sobre (4 px au repos) ; clé React stable sur la sculpture (sinon l'ouverture la remontait et coupait la lecture demandée au clic).
  - **Rien supprimé** : rig 2D (repli technique, visible au repos et pour la parole en direct), voix HD, boucle muette, page `/architecte` (cadre rond), carte Super-Admin (cadre rond et prévisualisation), Studio Avatar, réglages ; la présentation « en grand » de la barre est remplacée par la lecture dans la sculpture (décision de la Direction) ; l'égaliseur devient minimal (trois traits, vrai niveau).
* **Preuves** : `tests/architecteSequences.test.ts` (20 : empreintes des actifs détourés, calage borné, double appui, fin réelle « pause puis ended », `stop(slot)`, dates), `tests/architecteSequenceCutout.test.tsx` (2 : recomposition, couche attachée), barre (4 nouveaux : sculpture seule, clic → parle puis accueil et écoute différés, rejouer + micro fermé pendant la vidéo, flèche), écran et page mis à jour ; typage 0 erreur ; suite complète 1 506 tests / 98 fichiers (1 562 tests / 101 fichiers après la fusion de `main` d5740cd, composeur A7) ; build ✓ ; couche « Miroir d’eau » régénérée.
* **Essai navigateur réel (🗄️ banc `design-lab/banc/sculpture.html` servi par vite, composant réel posé sur une page qui ressemble à MokNet — six cartes de contenu, dock de navigation sur téléphone —, Chromium de preuve, réseau externe coupé)** — TEST_SET : ordinateur 1280 × 800 et téléphone 390 × 844 ×2 tactile ; METRIC : position, **part de l'écran couverte par l'interface de l'Architecte**, dock libre et cliquable, cartes dégagées, état du lecteur, `currentTime`, pixels du canevas, présence, ordre accueil / écoute ; RESULT — **obstruction** : ordinateur 0,9 % au repos, 2,4 % barre ouverte, 7,8 % panneau déplié ; téléphone 2,1 %, 6,8 %, 20,2 % (panneau à la demande) ; dock libre et cliquable dans tous les états ; cartes dégagées 6/6 (ordinateur), 5/6 puis 4/6 avec le panneau (téléphone) ; au repos seule la sculpture est visible, détourée (masque posé), à 24 px du bord droit et du bas sur ordinateur (12 / 96 px sur téléphone : au-dessus du dock) ; au clic, `playing` dans le geste, source `vision-smart-heygen.cutout.webm` (720 × 1440), canevas opaque, présence « parle », statut « Présentation en cours… », **46 % des pixels du canevas transparents et 50 % opaques** (détourage réel), barre ouverte, panneau replié ; fin réelle à 8,17 s (`ended` vrai, état `idle` après 8,6 s), puis l'accueil de première rencontre est prononcé (voix de secours du navigateur de preuve) et l'écoute demandée (le micro du banc est faux : « Le micro n'a pas démarré — utilisez la saisie », attendu) ; la flèche déplie le panneau avec l'accueil, au-dessus de la barre (téléphone : au-dessus de la sculpture) ; un clic sur la sculpture referme tout, la sculpture reste ; 0 erreur de page, 0 erreur média (seules ressources externes coupées). Captures `livrables/sculpture/{ordinateur,mobile}-{1..5}.png` remises à la Direction.
* **Limites dites** : la parole en direct (réponses de l'assistant) reste portée par le rig 2D détouré — aucune vidéo ne peut être générée à la volée pour un texte arbitraire ; le détourage est exact pour la phrase validée (matte par image) et statique pour le portrait (masque unique, la tête du rig bouge peu) ; le banc a joué le WebM sur un Chromium sans H.264 — le MP4 empilé (H.264 High 4.1) est celui des autres navigateurs ; Safari/iOS non exercé sur appareil réel ; le préchargement de la vidéo empilée est limité aux métadonnées.
* **Collision de numérotation, dite et résolue** : la PR #95 (compétence « production contrôlée ») et la branche avatar (PR #71) ont pris toutes deux DEC-2026-062 — la PR #95 a été fusionnée à 13:00:16 UTC, entre ma dernière vérification de `main` (12:59) et ma fusion (13:00:55). L'équipe de la compétence déplace son entrée vers **DEC-2026-074** (PR #97, en cours avant la présente PR : elle termine d'abord, règle « une seule fusion à la fois ») ; les entrées de la branche avatar **restent DEC-2026-062 → 073**, et la correction « zéro obstruction strict » prend DEC-2026-075. Un premier décalage +1 (063 → 074) préparé ici a été retiré avant fusion pour ne pas recréer la collision. Leçons : contrôler `main` **dans la même minute** que la fusion, et lire les PR ouvertes avant de renuméroter.
* **Statut** : 🟢 **EN PRODUCTION CONTRÔLÉE depuis le 5/09/2026 à 13:01 UTC** — consigne de coordination respectée (aucune autre fusion ni déploiement en cours au moment de la fusion : `main` sur d5740cd, déploiement Netlify « ready ») ; PR #71 fusionnée en squash → `main` `d9c2c2c` (Green Gate vert sur `9dd10f1`, run 33967521429) ; `moknet.net` sert `index-DnzvQMzK.js` (marqueurs de la sculpture présents : ancrage, vidéo détourée, masque de silhouette, flèche ; ancien bundle `index-ggiwFepQ.js` → 404) ; actifs détourés servis en production avec les tailles et empreintes attendues ; page publique `/architecte` vérifiée en Chromium réel (lecture réelle jusqu'à la fin, 0 erreur JS) ; page d'accueil rendue sans erreur. Non vérifié en production faute de compte de banc : la sculpture flottante après connexion (le composant est celui prouvé au banc et dans les tests) — à confirmer par la Direction à sa prochaine connexion.

### [DEC-2026-072] — 5 Septembre 2026
* **Module(s)** : Diallo OS & Architecte (module 01), Super-Admin. (Aucun code du Studio & Création (module 11) n'est touché par cette décision — mention retirée à la relecture indépendante.)
* **Problème / Besoin initial** : Mission ferme de la Direction (05/09, après la validation de l'avatar HeyGen) : « Tu prends strictement le modèle de la capture comme base validée. Tu ne changes pas de modèle, tu n'en crées pas un autre, aucune régression. Tu l'améliores sans rien supprimer. Tu peux ajouter si c'est plus confortable. »
* **Compétences de référence** : AI Core playbook 15 v1.1.0 (niveau P3a « vidéo parlante pré-rendue », repli P2 puis P1, pastille média synthétique, aucun démarrage automatique d'un contenu sonore), playbook 16 (méthode validée, fichiers livrés avec l'application, cache par phrase, plafond de crédits), ADR-0025, agent `AVATAR_CREATION_INTEGRATION_AGENT` (W21) ; playbook 09 (mouvement utile, accessibilité) ; playbook 04 (preuves).
* **Décision retenue — le modèle validé devient la base ferme, intégré par AJOUT** :
  - **Registre des séquences** (`services/architecte/sequences.ts`) : la vidéo HeyGen validée (phrase Vision Smart, expressivité moyenne, 720p, portrait officiel, voix HD ElevenLabs « Claire ») est livrée comme fichiers statiques `public/architecte/vision-smart-heygen.mp4` (H.264 + AAC, 2 457 265 octets) et `.webm` (VP9 + Opus, transcodé du même fichier, pour les navigateurs sans H.264), avec poster `.webp` et légendes `.fr.vtt` générées depuis les mêmes repères que les sous-titres ; **empreintes SHA-256 et tailles vérifiées par les tests** : un fichier remplacé sans passer par le registre fait échouer la suite. Rien n'est généré à l'exécution : aucune clé, aucun crédit, aucun appel fournisseur côté produit.
  - **Lecteur** (`createSequencePlayer`) : machine d'états `idle → loading → playing → ended → idle` / `failed` ; lecture demandée **dans le geste** (synchrone au clic, sinon le son est refusé sur mobile) ; choix du cadre par emplacement nommé (« demo », « presentation », « admin-preview ») ; toute fin, erreur, pause extérieure ou démontage **rend la main au rig 2D** ; jamais de démarrage automatique.
  - **Couche vidéo dans le cadre** (`ArchitecteSequenceVideo`, prop `sequence` de `ArchitecteAvatar`) : `<video playsinline>` par-dessus le portrait vivant, invisible au repos, fondu à l'apparition, MP4 puis WebM, piste de légendes française ; attribut `data-sequence` observable ; le rig continue dessous, prêt à reprendre.
  - **Page `/architecte`** : nouveau bouton « Voir l'avatar vidéo (modèle validé) » à côté des commandes existantes (conservées) ; état « parle », phrase et provenance affichées ; message honnête en cas d'échec (« L'avatar animé reste actif »).
  - **Barre flottante** : bouton « Présentation » dans la barre ouverte ; invitation « Nouveau » discrète **une fois par appareil** (mémorisée), sans ouvrir le panneau de texte (§16-17 respecté) ; la présentation s'ouvre dans la surface visuelle existante avec l'avatar en grand (224 px) ; la voix de l'assistant est coupée avant ; fermer la fenêtre arrête la vidéo.
  - **Super-Admin** : réglage `videoSequencesEnabled` (absent des réglages enregistrés = activé), liste des séquences (durée, modèle, date et auteur de validation), prévisualisation dans l'aperçu de la carte.
  - **Rien supprimé** : rig 2D, voix HD, boucle muette, Studio Avatar, réglages existants ; le niveau de présence en direct reste P2, la vidéo ajoute P3a.
* **Preuves** : `tests/architecteSequences.test.ts` (14 : registre, empreintes, légendes, machine d'états, lecteur, invitation), écran (+5), page `/architecte` (+3), barre (+1) ; typage 0 erreur ; suite complète 1 422 tests / 92 fichiers verte ; build ✓ ; couche « Miroir d'eau » régénérée par l'outil du dépôt.
* **Essai navigateur réel (🗄️ build de la branche servi localement, Chromium de preuve, réseau externe coupé ; la prévisualisation Netlify sert les mêmes fichiers, HTTP 200 vérifié)** — BASELINE : page `/architecte` avant clic (vidéo présente, invisible, `idle`) ; TEST_SET : ordinateur 1280 × 900 ×2 et mobile 390 × 844 ×3 tactile ; METRIC : état du lecteur, `currentTime`, opacité, présence, texte d'état, fin, erreurs média ; THRESHOLD : lecture réelle jusqu'à la fin, vidéo visible pendant la lecture, retour au repos, 0 erreur média ; RESULT : chargement 0,55 s / 0,52 s, clic → `playing`, opacité 0,97 / 0,98 dès la première image, présence « parle », état « L'Architecte parle », `currentTime` 8,2 s, `ended` vrai, retour `idle`, 0 erreur média sur les deux appareils ; source choisie par ce Chromium : le WebM (il ne décode pas H.264), ce qui prouve aussi le repli de source ; les navigateurs courants prendront le MP4.
* **Limites dites** : une seule séquence (la phrase validée) ; toute nouvelle phrase se produit avec la méthode du playbook 16 puis se livre après validation ; le Chromium de preuve ne décode pas H.264 (le WebM sert aussi de preuve) ; l'identité HeyGen est légèrement re-synthétisée par rapport au portrait du rig (fondu de 300 ms) ; pas de génération à la volée par la passerelle (prochaine étape, sur autorisation).
* **Statut** : 🟠 en prévisualisation de branche, fusion contrôlée préparée — la fusion dans `main` déploie automatiquement sur moknet.net et attend le feu vert explicite de la Direction.

### [DEC-2026-071] — 5 Septembre 2026
