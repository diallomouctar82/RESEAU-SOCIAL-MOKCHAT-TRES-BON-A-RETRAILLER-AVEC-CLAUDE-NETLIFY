...
* **Autre outil déjà accessible** : OmniHuman via arcade lancé sur le même portrait et le même son (800 crédits arcade débités sur le solde existant, ~11 min de génération) — résultat consigné dès réception.
...
* **Statut** : 🟢 **Avatar HeyGen validé par la Direction** ; méthode capitalisée dans AI Core ; intégration produit NON réalisée, NON déployée, rien acheté.
