...
* **Collision de fusion, dite et corrigée** : la PR #95 (compétence « production contrôlée », DEC-2026-062) a été fusionnée à 13:00:16 UTC, entre ma dernière vérification de `main` (12:59, tête d5740cd) et ma fusion (13:00:5x) ; Git a fusionné les deux sans conflit ni écrasement (les deux lots sont intacts sur `main`), mais les deux ont pris le numéro DEC-2026-062. Correction par cette PR de mémoire vivante : les entrées de la branche avatar passent de DEC-2026-062 → 073 à **DEC-2026-063 → 074** (les versions v6.27.0 → v6.37.0 ne changent pas, la PR #95 n'ayant pas pris de version). Leçon (compétence « production contrôlée » § 7.4) : vérifier `main` dans la même minute que la fusion, et poser la fusion sur l'API avec la tête de `main` attendue.
