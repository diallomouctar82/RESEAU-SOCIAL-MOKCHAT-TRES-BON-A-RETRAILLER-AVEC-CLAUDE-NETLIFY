...
* **Décision retenue — verrou d'entrée fail-closed + domaine canonique déclaré** :
  - `services/auth.ts#verifierSession(session)` : `GET /auth/v1/user` avec le jeton de la session locale (passé explicitement, sans verrou multi-onglets). Verdict `valide` (utilisateur confirmé, même identifiant) ; `invalide` (refus du serveur 401/403, utilisateur absent ou différent, session locale incomplète) → `signOut({ scope: 'local' })` puis écran de connexion ; `non-verifiee` (serveur injoignable, 5xx, ou 8 s sans réponse — `DELAI_VERIFICATION_SESSION_MS`) → la session locale **non expirée** est conservée : tolérance DITE pour un réseau mobile capricieux, qui ne vaut jamais pour un jeton que le serveur a refusé.
  - `App.tsx` : les sessions relues depuis le stockage (`getSession()` au démarrage et l'événement `INITIAL_SESSION`, qui rejoue le même jeton) passent par ce verdict, **une seule vérification par jeton** (verdict partagé) ; `SIGNED_IN`, `TOKEN_REFRESHED`, `USER_UPDATED` portent une session que le serveur vient d'émettre et entrent comme avant (aucun aller-retour ajouté à la connexion). Tout le reste est inchangé : entrée directe sur Réseau MokNet, déconnexion, récupération de mot de passe, module autonome, `/architecte`, « Se souvenir de moi ».
...
* **Preuves** : `tests/sessionValidee.test.ts` (9 : valide, refus 401, compte supprimé 403, utilisateur différent, panne réseau, 503, délai dépassé, session incomplète, effacement local qui échoue), `tests/appEntreeReseau.test.tsx` (+8 : aucune session, session refusée → connexion, session valide → Réseau MokNet, `INITIAL_SESSION` partage le verdict, refus par `INITIAL_SESSION` seul, connexion sans seconde vérification, serveur injoignable → tolérance dite, déconnexion puis reconnexion), `tests/netlifyRedirectsCanoniques.test.ts` (6 : trois règles 301 forcées, aucune boucle, réécritures existantes intactes, ordre) ; suite complète 1 611 tests / 106 fichiers ✓ ; typage 0 (relancé après les tests) ; build ✓.
...
* **Statut** : 🟡 **PRÊTE POUR PRODUCTION CONTRÔLÉE** — PR brouillon ouverte sur `claude/moknet-public-access-auth-hu6yy7`, en attente du Green Gate, de la revue indépendante et du feu vert écrit de la Direction (v6.40.0).
