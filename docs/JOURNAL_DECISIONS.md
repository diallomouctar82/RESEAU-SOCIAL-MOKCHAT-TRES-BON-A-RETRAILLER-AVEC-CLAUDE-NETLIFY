...
* **Décision retenue — continuité de l'avatar sur toute la conversation, même périmètre, rien retiré** :
  ...
  - `speakWithElevenLabs` : première phrase refusée → `finishSpeakingElevenLabs(options, 'relais')` (ni `onEnd`, ni micro relancé) et `return false` → repli navigateur ; flux de capture micro relâché (`stopVolumeMonitoring()`) pendant la lecture HD et le repli (téléphone : plus de sortie en mode « appel »), rouvert par `startListening` à la fin.
