...
* **Décision retenue — la sculpture vivante remplace le bouton, le modèle validé parle dedans** :
  ...
  - **Rien supprimé** : rig 2D (repli technique, visible au repos et pour la parole en direct), voix HD, boucle muette, page `/architecte` (cadre rond), carte Super-Admin (cadre rond et prévisualisation), Studio Avatar, réglages ; la présentation « en grand » de la barre est remplacée par la lecture dans la sculpture (décision de la Direction).
