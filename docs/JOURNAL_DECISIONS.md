...
* **Limites dites** : navigateurs émulés, pas d'appareil physique — le contrôle sur téléphone et tablette réels appartient à la Direction ; la reprise au retour sur la page est doublée (rejeu `SIGNED_IN` de supabase-js + relecture de l'écran) : deux vérifications du même jeton au pire, partagées quand elles se chevauchent ; le résidu `lmav_chat_conversations_cache` après déconnexion relève du chantier messagerie (décision Direction).
