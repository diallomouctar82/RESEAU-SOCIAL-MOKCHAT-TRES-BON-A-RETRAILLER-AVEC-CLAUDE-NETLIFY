...
| Présente, non expirée, **aucun verdict** (panne réseau, 5xx, 429, réponse non JSON d'un portail captif ou d'un proxy, 8 s sans réponse) | `non-verifiee` | **Écran de reprise** « MokNet est momentanément injoignable, veuillez réessayer. » — jamais l'interface, session locale **conservée** (non refusée), reprise sans ressaisie dès que le serveur répond (DEC-2026-083 ; avant : tolérance dite, l'interface s'ouvrait vide) |
| Présente, **expirée**, serveur injoignable | `relireSession()` → `injoignable` (supabase-js n'a pas pu rafraîchir : `AuthRetryableFetchError`, session gardée dans le stockage) | Écran de reprise **sous 8 s** (relecture plafonnée ; avant : roue de chargement ≈ 25 s puis écran de connexion) ; au retour du réseau, le rafraîchissement aboutit à la tentative suivante **hors de la fenêtre de 60 s** pendant laquelle supabase-js garde en cache l'échec du même jeton (`REFRESH_FAILURE_COOLDOWN_MS`, tentative programmée juste après ; au pire ≈ 1 min après le retour du réseau), puis le jeton est vérifié et l'interface s'ouvre sans ressaisie |
...
- **Écran de reprise** (`components/ServeurInjoignable.tsx`, DEC-2026-083) : bouton « Réessayer » (une seule tentative à la fois, partagée avec la relecture initiale, l'événement `online` et la minuterie), reprise automatique à l'événement `online`, toutes les 30 s (`INTERVALLE_REPRISE_MS`) et une fois juste après la fenêtre de 60 s du cache d'échec de supabase-js (`DELAI_CACHE_ECHEC_RAFRAICHISSEMENT_MS`, garde-fou dans `tests/sessionValidee.test.ts`) ; au retour sur la page, c'est le rejeu `SIGNED_IN` de supabase-js (jeton valide) ou sa nouvelle tentative de rafraîchissement (jeton expiré) qui passe par le même verrou — pas de second écouteur, qui doublait le chargement du profil ; un verdict « non vérifié » n'est jamais mémorisé (chaque tentative interroge le serveur ; un jeton refusé reste refusé) ; l'`INITIAL_SESSION` nul émis par supabase-js quand son rafraîchissement échoue hors ligne ne ferme pas l'écran ; `SIGNED_OUT`, la déconnexion ou un refus du serveur le ferment. Sans session sur l'appareil, l'écran de connexion reste l'entrée, serveur joignable ou non.
...
**Limites dites** : un jeton d'accès encore valide (au plus 1 h) après une « déconnexion partout » depuis un autre appareil n'est pas révoqué par ce contrôle ; quand le client vient de rafraîchir le jeton à l'ouverture, ce jeton neuf est vérifié une fois de plus (un appel, bénin) ; la tolérance « serveur injoignable » a été fermée par décision de la Direction (DEC-2026-083) ; `/architecte` reste la page publique de démonstration (DEC-2026-066, confirmée le 05/09 après audit : la page elle-même ne lit que la configuration de l'avatar en mémoire, joue un fichier audio statique et la voix du navigateur, n'écrit rien, n'appelle aucun service payant ; l'enveloppe applicative — `adminConfigService`, construite au chargement du bundle sur toute route, écran de connexion compris — émet une lecture de `profiles` filtrée par la RLS (vide sans session) et ouvre un canal Realtime : préexistant, hors périmètre, arbitrage séparé proposé) ; avec un jeton d'accès **expiré**, une réponse du serveur autre que 5xx/réseau sur `/auth/v1/token` (429, 400 inattendu) fait retirer la session par supabase-js lui-même, hors du contrôle de l'application (limite de la bibliothèque).
