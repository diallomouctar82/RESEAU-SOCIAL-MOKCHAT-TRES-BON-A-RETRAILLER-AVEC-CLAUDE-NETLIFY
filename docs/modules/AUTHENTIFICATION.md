...
- **Écran de reprise** (`components/ServeurInjoignable.tsx`, DEC-2026-083) : bouton « Réessayer » (une tentative à la fois), reprise automatique à l'événement `online`, au retour sur la page (`visibilitychange` — en plus du rejeu `SIGNED_IN` de supabase-js, qui passe par le même verrou) et toutes les 30 s ; un verdict « non vérifié » n'est jamais mémorisé (chaque tentative interroge le serveur ; un jeton refusé reste refusé) ; l'`INITIAL_SESSION` nul émis par supabase-js quand son rafraîchissement échoue hors ligne ne ferme pas l'écran ; `SIGNED_OUT`, la déconnexion ou un refus du serveur le ferment. Sans session sur l'appareil, l'écran de connexion reste l'entrée, serveur joignable ou non.
