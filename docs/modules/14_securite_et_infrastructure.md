...
- **Fichiers Clés** :
  ...
  - `supabase/functions/health-guardian/` : fonction Edge de sondage (`index.ts` seul au réseau, `evaluate.ts` et `liveTransportProbe.ts` purs), artefact déployé généré par `build-bundle.sh` (v3 le 5/09/2026 : sondes CORS des six fonctions, façade HTTPS et porte `/rtc` du VPS, compteurs `super_admin`/`admin` ; repli CORS jamais `*`).
