...
## 🧑‍🚀 AVATAR VIVANT DE L'ARCHITECTE (DEC-2026-063 → DEC-2026-073, 04–05/09/2026)
...
- **Synchro labiale à trois niveaux, jamais surévalués** : `amplitude_reelle` (toute voix HD servie par la chaîne vocale — amplitude efficace **temporelle** mesurée sur l'élément `<audio>`, normalisée par une crête adaptative, bouche close sous 16 % de la crête ; la voix entendue est retardée de 60 ms pour que la bouche ne soit jamais en retard — corrélation son ↔ bouche mesurée 0,80 à 0 ms sur la phrase Vision Smart, DEC-2026-066), `rythme_des_mots` (moteur natif — `speechSynthesis` n'expose aucun flux audio ; frontières de mots relayées jusqu'à l'avatar), `aucune`. Tous les lissages (bouche, emphase, part de parole, attention, largeur de lèvres) sont des **constantes de temps**, pas des facteurs par image.
- **Visèmes acoustiques** (DEC-2026-067, `spectralBands` → `mouthShapeFromBands`) : la FORME de la bouche vient du spectre de la voix à chaque image — mâchoire ouverte sur « a » (premier formant haut), presque close sur « i »/« ou », lèvres étirées (second formant haut) ou arrondies, dents sur les fricatives, lèvres jointes sur « m »/« b »/« p » et les silences ; ouverture de parole plafonnée à 60 % de la course (`MAX_SPEECH_OPENNESS`). Publiée par `voiceEngine.onMouthShape` jusqu'à l'avatar (`mouthShapeRef`).
- **Anticipation et coarticulation** (DEC-2026-068, `MouthShapeBuffer`, `mouthReadTime`) : les lèvres précèdent le son entendu de 60 ms (`VISUAL_LEAD_MS`), formes moyennées sur ±40 ms sans retard de phase ; retard du son 200 ms (`LIP_SYNC_LOOKAHEAD_MS`), retard d'affichage mesuré 80 ms (`RENDER_LATENCY_MS`) et latence de sortie de l'appareil déduits. Mesuré : −67 ms au montage, −69 ms en temps réel. Pendant la parole, dérive et inclinaison de repos s'effacent ; clignements aux pauses de la voix seulement.
- **Visèmes phonétiques alignés** (DEC-2026-069, `services/architecte/phonemes.ts` + `alignment.ts`) : le texte que la chaîne vocale va dire est transcrit en phonèmes par règles (lexique des mots grammaticaux et des marques, ponctuation gardée comme pauses, accents de groupe et de mot), puis **aligné sur le clip décodé** avant la lecture (Viterbi semi-markovien sur classes acoustiques : énergie dB, passages par zéro, part haute ; pauses de ponctuation obligatoires, blancs facultatifs, segment joker) — 67 à 114 ms pour 8 s de son, en tâche de fond. La piste (phonèmes, mots, niveau, images-clés coarticulées) donne la bouche à l'instant qui sera entendu (`trackShapeAt` sur `audio.currentTime` lissé, `MEDIA_PIPELINE_MS`), niveau `visemes_alignes` annoncé ; repli à l'amplitude mesurée si le décodage ou l'alignement échoue.
- **Partition des gestes** (DEC-2026-069, `buildProsodyScore`, `updateScore`) : hochement 90 ms **avant** la syllabe accentuée, sourcils et relèvement 150 ms avant le premier mot d'une phrase, regard qui s'échappe avant une phrase sur deux, clignements dans les pauses de ponctuation et pendant les saccades (écart ≥ 1,2 s, secours reporté à la pause suivante), inclinaison à chaque point, tête qui suit le regard (`turnX`) ; couplage yeux-tête dans la pose (les yeux restent sur l'interlocuteur quand la tête hoche). Sans piste, le suiveur réactif (DEC-2026-067) reste le repli — sur une seule horloge (défaut corrigé : le clignement de parole n'était jamais joué dans le navigateur).
- **Stabilité mesurée** (DEC-2026-070, local) : `CANVAS_PIXEL_BUDGET` (420 × 420) borne la résolution du canevas et `adaptQuality` la baisse d'un cran quand la durée médiane des images dépasse 40 ms (60 images/s au lieu de 45 sur le navigateur de preuve) ; hochements lents (`NOD_OMEGA` 13 rad/s) et rares (accent de groupe), couplage yeux-tête discret (0,2), regard qui s'échappe deux fois plus petit, balancement à deux périodes, respiration 1,1 %, fermeture de bouche continue, ressorts hérités d'un segment à l'autre ; **lèvres naturelles** : coarticulation anticipatrice dans la piste (`coarticulatedTarget`), coins des lèvres arrondis/étirés par le peintre (`drawLipCorners`), lèvres scellées 20 ms avant le silence, `MOUTH_RELEASE_MS` 48, `VISUAL_LEAD_MS` 40.
- **Le modèle validé, intégré par ajout** (DEC-2026-072, v6.36.0) : la séquence vidéo HeyGen validée par la Direction le 05/09 (phrase Vision Smart, expressivité moyenne, 720p, voix HD) est livrée avec l'application (`public/architecte/vision-smart-heygen.mp4` + `.webm` + poster + légendes, empreintes vérifiées par `tests/architecteSequences.test.ts`) et jouée **dans le cadre même de l'avatar** par `ArchitecteSequenceVideo` (prop `sequence` de `ArchitecteAvatar`, lecteur à états `createSequencePlayer` dans `services/architecte/sequences.ts`) ; entrées : bouton « Voir l'avatar vidéo (modèle validé) » sur `/architecte`, bouton « Présentation » de la barre flottante (invitation « Nouveau » une fois par appareil, jamais de démarrage automatique, fenêtre visuelle avec l'avatar en 224 px), réglage Super-Admin `videoSequencesEnabled` + prévisualisation. Le rig 2D reste le repli automatique et le seul moteur de la parole en direct : rien n'est supprimé. Niveau P3a (playbook 15 v1.1.0, ADR-0025 d'AI Core) ; le pilote et la méthode sont dans DEC-2026-071 et le playbook 16.
- **La sculpture vivante** (DEC-2026-073, v6.37.0) : `ArchitecteAvatar` variante `sculpture` (masque de silhouette CSS sur le portrait vivant, lueur d'état, calage vidéo ↔ portrait `SequenceAlignment`), `ArchitecteSequenceCutout` (vidéo empilée couleurs + matte `vision-smart-heygen.cutout.mp4|webm`, recomposée image par image dans un canevas) ; la barre flottante devient une petite barre d'icônes ancrée à côté de la sculpture (ancrage unique `architecte-ancrage`, colonne étroite, panneau ≤ 22 rem × 40 vh à la demande — zéro obstruction mesurée) : la sculpture est le bouton permanent, elle parle au clic (`ouvrirParLaSculpture`, cadre `SCULPTURE_SLOT`), l'accueil et l'écoute sont différés à la fin de la vidéo (`pendingGreetingRef`, `resumeListeningRef`, grâce `PRESENTATION_START_GRACE_MS`), flèche `isPanelOpen` à trois états ; banc de preuve `design-lab/banc/sculpture.html` ; méthode de détourage capitalisée dans AI Core (playbook 16).
