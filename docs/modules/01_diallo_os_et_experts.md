...
## 🧑‍🚀 AVATAR VIVANT DE L'ARCHITECTE (DEC-2026-060 → DEC-2026-069, 04–05/09/2026)
...
- **Le modèle validé, intégré par ajout** (DEC-2026-069, v6.33.0) : la séquence vidéo HeyGen validée par la Direction le 05/09 (phrase Vision Smart, expressivité moyenne, 720p, voix HD) est livrée avec l'application (`public/architecte/vision-smart-heygen.mp4` + `.webm` + poster + légendes, empreintes vérifiées par `tests/architecteSequences.test.ts`) et jouée **dans le cadre même de l'avatar** par `ArchitecteSequenceVideo` (prop `sequence` de `ArchitecteAvatar`, lecteur à états `createSequencePlayer` dans `services/architecte/sequences.ts`) ; entrées : bouton « Voir l'avatar vidéo (modèle validé) » sur `/architecte`, bouton « Présentation » de la barre flottante (invitation « Nouveau » une fois par appareil, jamais de démarrage automatique, fenêtre visuelle avec l'avatar en 224 px), réglage Super-Admin `videoSequencesEnabled` + prévisualisation. Le rig 2D reste le repli automatique et le seul moteur de la parole en direct : rien n'est supprimé. Niveau P3a (playbook 15 v1.1.0, ADR-0025 d'AI Core) ; le pilote et la méthode sont dans DEC-2026-068 et le playbook 16.
