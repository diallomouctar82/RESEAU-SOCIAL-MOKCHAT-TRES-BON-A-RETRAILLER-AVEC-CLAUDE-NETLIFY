...
- **La sculpture vivante** (DEC-2026-072, v6.36.0) : `ArchitecteAvatar` variante `sculpture` (masque de silhouette CSS sur le portrait vivant, lueur d'état, calage vidéo ↔ portrait `SequenceAlignment`), `ArchitecteSequenceCutout` (vidéo empilée couleurs + matte `vision-smart-heygen.cutout.mp4|webm`, recomposée image par image dans un canevas) ; la barre flottante n'a plus de visage dans la barre : la sculpture est le bouton permanent, elle parle au clic (`ouvrirParLaSculpture`, cadre `SCULPTURE_SLOT`), l'accueil et l'écoute sont différés à la fin de la vidéo (`pendingGreetingRef`, `resumeListeningRef`, grâce `PRESENTATION_START_GRACE_MS`), flèche `isPanelOpen` à trois états ; banc de preuve `design-lab/banc/sculpture.html` ; méthode de détourage capitalisée dans AI Core (playbook 16).
