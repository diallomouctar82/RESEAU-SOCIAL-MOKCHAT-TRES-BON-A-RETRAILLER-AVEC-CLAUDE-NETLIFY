...
- **Fichiers Clés** :
  ...
  - `components/MoocChatFloating.tsx` : Messagerie privée directe persistée et connectée en temps réel à Supabase (`conversations`, `conversation_participants`, `messages` — réécrite au LOOP 06/17, mission Architecte MOCnet, contre le vrai schéma ; les noms `chat_conversations`/`chat_messages` d'une version antérieure de cette fiche n'ont jamais existé en base). Pour chaque envoi, `messages.content` reste le texte original et `metadata.original_language` mémorise la langue choisie par l'auteur.
  - `services/translation/translationService.ts` : service transversal unique de traduction. La messagerie ne connaît aucun fournisseur et appelle seulement `translationService.translateText(...)`; le moteur peut être remplacé via le contrat `TranslationEngine` sans toucher les composants. Cache exclusivement mémoire, déduplication des demandes en vol et repli honnête vers l'original en cas d'indisponibilité.
  - `components/chat/ChatMessageItem.tsx` : Rendu riche des bulles de messages (textes, photos, vidéos, documents téléchargeables, messages vocaux avec lecteur waveform HD, réactions emoji réellement persistées, citations). Un message reçu est traduit automatiquement lorsqu'il devient visible dans la langue `profiles.preferred_language` du lecteur ; l'original reste affiché au-dessus et n'est jamais remplacé ni supprimé. L'épinglage affiché dans l'UI n'est pas câblé à une donnée réelle (aucune colonne `is_pinned` sur `messages`) — non fonctionnel.
...
- **Invariance de l'original** : la traduction est une vue dérivée pour le lecteur. Elle ne modifie jamais `messages.content`, n'est pas écrite à la place de la source et ne masque pas le texte de l'auteur. En cas de panne du moteur, l'original reste immédiatement lisible.
...
- **Terminé** :
  ...
  - Messagerie privée en temps réel (Supabase Realtime + envoi de médias/messages vocaux + réactions + réponses + résumé + traduction texte automatique via le service central + modération/signalement Super-Admin). **Correctif production du 1er septembre 2026 conservé** : les identifiants de conversations locales (`chat-…`/`local-…`) restent dans le mode local et sont bloqués avant les API Supabase typées UUID; les conversations réelles conservent leur historique et leurs abonnements Realtime. **Livré depuis (1er–3 septembre 2026, v6.7.0 → v6.12.0)** : « Ma langue » unique avec détection automatique et vocaux transcrits (UL/HL) ; appels audio/vidéo réels sur le serveur LiveKit de production, audio bidirectionnel **validé par l'utilisateur sur deux téléphones** (AU) ; voix traduite dans l'appel, appel normal par défaut, langue choisie par appel (VT‑1b) ; connexion quasi immédiate et traduction dès les premiers mots (LT) ; sonnerie et notification fiables appli fermée, bouton « Sonnerie », appel entrant au premier plan (SN). Détail vérifié par mission dans `docs/APPELS_AUDIO_VALIDATION_APPAREILS.md` ; état exact des tables et des pièces jointes dans `docs/SUPABASE_ARCHITECTURE.md` (lignes Messagerie, VF, AU, VT, LT, SN).
  - Feed social, Stories, Lecteur de Reels avec produits liés.
  - **Espace Live Intelligent 100% Opérationnel** : Création instantanée et programmation sans écran blanc, streaming WebRTC/Hardware résilient, sous-titrage bilingue en temps réel, copilotes IA Diallo OS, tableau blanc interactif, intégration des actions d'apprentissage/projet et compte-rendu post-session. **LV-2 point 1 (5 septembre 2026, v6.43.0, DEC-2026-082, en production contrôlée)** : le bouton de sortie du Studio porte un libellé texte VISIBLE « Quitter » (téléphone et ordinateur) et sort proprement selon le rôle — le spectateur quitte immédiatement (média coupé, session quittée, vue refermée sans recharger la page), l'animateur garde son compte-rendu de fin (`handleEndLive` inchangé).
