...
> *Version Courante : v6.18.0 (**SAT-4 EN COURS DE LIVRAISON — v6.19.0 : LA SANTÉ GLOBALE SAIT DÉSORMAIS SI UN DIRECT PEUT VRAIMENT DÉMARRER, FONCTION EDGE `health-guardian` v2 EN PRODUCTION ET DÉMONTRÉE, LIGNE CLIENT EN PR** ; « RÉSEAU MOC » SOUS « ACCUEIL », CONTOURS DES ZONES DE SAISIE RENFORCÉS, NOUVELLE INVITE DU COMPOSEUR — PRODUCTION CONTRÔLÉE ; BARRE LATÉRALE D’« ACCUEIL » À « CONSEIL DES SAGES » ; ACCUEIL NETTOYÉ — SIX DÉCLENCHEURS RETIRÉS DE L'AFFICHAGE, AUCUNE FONCTION SUPPRIMÉE, VALIDÉ PAR LA DIRECTION ; MENU « MIROIR D'EAU » + STUDIO LIVE « ABYSSE » **EN PRODUCTION SUR `moknet.net`, VALIDÉS PAR LA DIRECTION** ; MENUS DE L'EN-TÊTE RÉPARÉS ; APPELS AUDIO/VIDÉO RÉELS, TRADUITS VOIX À VOIX, SONNERIE FIABLE APPLI FERMÉE, SÉCURITÉ BASE RENFORCÉE ; **SATURATION DES DIRECTS : CODE CLIENT EN PRODUCTION ET FONCTION EDGE EN VERSION 7, MAIS ENCORE DÉLIBÉRÉMENT INERTE** — voir plus bas)*
...
**5 septembre 2026 — SAT-4 (DEC-2026-054, v6.19.0 en cours de livraison).** Le tableau de bord « Santé Globale » du Super-Admin sait désormais **si un direct peut réellement démarrer**, et non plus seulement si le serveur répond : la ligne `live.transport_utilisable` appelle `ListRooms` sur le serveur LiveKit avec la clé du coffre — l'appel dont dépend l'ouverture de chaque direct — et distingue un serveur qui répond mais refuse nos identifiants (rouge), un serveur trop lent pour que la porte d'admission SAT-2 puisse compter (orange, au-delà de 1 500 ms), un serveur injoignable (rouge) et un serveur utilisable (vert). Ne pas avoir sondé reste blanc, jamais vert. **En production** : la fonction Edge `health-guardian` v2, démontrée le 5 septembre à 00h10 UTC avec une vraie session administrateur (HTTP 200 en 2,17 s, 41 lignes évaluées, `live.transport_utilisable` **vert**, 400 ms, 0 direct en cours, preuve réelle) ; son artefact de déploiement est maintenant généré par `build-bundle.sh` et relu identique depuis la production. **Pas en production** : la ligne côté écran (code client) est sur la branche `claude/lives-directs`, en PR — l'onglet l'affichera après fusion et déploiement Netlify. **Partiel** : aucune contre-épreuve n'a été jouée sur le vrai serveur (la faire passer au rouge exigerait de casser la clé du coffre ; les contre-épreuves vivent dans les tests, au niveau de la règle) ; SAT-5, SAT-6 et SAT-7 ne sont pas commencés ; les étapes ACT-3/4/5 du plan d'activation restent bloquées sur l'accès SSH au VPS. Le compte administrateur éphémère qui a servi à la démonstration a été supprimé avec balayage vérifié à zéro ; le seul administrateur en base est le compte réel de la Direction.
