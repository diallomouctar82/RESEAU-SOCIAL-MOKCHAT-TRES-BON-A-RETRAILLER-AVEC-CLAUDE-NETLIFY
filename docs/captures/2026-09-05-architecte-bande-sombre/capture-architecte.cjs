...
const CHROMIUM = process.env.CHROMIUM || '/opt/pw-browsers/chromium-1194/chrome-linux/chrome'; // build chromium-1194 (Playwright 1.56)
...
(async () => {
  ...
  for (const [nom, w, h, dpr] of ECRANS) {
    ...
    if (await bouton.count()) {
      ...
      // Le sommet du cadre rond doit être à l'écran sur la capture (revue indépendante, constat 7).
      await canvas.scrollIntoViewIfNeeded(); await p.evaluate(() => window.scrollBy(0, -24)); await p.waitForTimeout(300);
