...
const SCENARIOS = {
  ...
  'session-reseau-coupe': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE (fermé par défaut, DEC-2026-083 : session locale non expirée, serveur injoignable)' },
  'session-reseau-coupe-puis-retour': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', apres: { mode: 'valide', evenement: 'online', attendu: 'INTERFACE_INTERNE' } },
  'session-reseau-coupe-puis-bouton': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', apres: { mode: 'valide', action: 'bouton', attendu: 'INTERFACE_INTERNE' } },
  'session-reseau-coupe-puis-visibilite': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', apres: { mode: 'valide', evenement: 'visibilitychange', attendu: 'INTERFACE_INTERNE' } },
  'session-reseau-coupe-bouton-toujours-coupe': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', apres: { mode: 'coupe', action: 'bouton', attendu: 'ECRAN_REPRISE', attenteMs: 12000 } },
  'session-reseau-coupe-puis-refus': { stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', apres: { mode: 'refus-401', action: 'bouton', attendu: 'ECRAN_AUTHENTIFICATION', attenteMs: 12000 } },
  'lien-prive-live-reseau-coupe': { chemin: '?live=live-banc-123', stockage: { [CLE]: JSON.stringify(sessionLocale) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE' },
  ...
  'session-expiree-serveur-injoignable': { stockage: { [CLE]: JSON.stringify(sessionExpiree()) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', attenteMs: 60000, chrono: true },
  'session-expiree-serveur-injoignable-puis-retour': { stockage: { [CLE]: JSON.stringify(sessionExpiree()) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', attenteMs: 60000, chrono: true, apres: { mode: 'valide', evenement: 'online', attendu: 'INTERFACE_INTERNE', attenteMs: 100000 } },
  'session-expiree-serveur-injoignable-retour-tardif': { stockage: { [CLE]: JSON.stringify(sessionExpiree()) }, reseau: 'coupe', attendu: 'ECRAN_REPRISE', attenteMs: 60000, chrono: true, apres: { delaiAvantMs: 35000, mode: 'valide', evenement: 'online', attendu: 'INTERFACE_INTERNE', attenteMs: 120000 } },
...
async function routerSupabase(ctx, etat, journal) {
...
(async () => {
  ...
  for (const sc of scenarios) {
    ...
    for (const canal of canaux) {
      ...
      const journal = [];
      const etat = { mode: scen.reseau };
      await routerSupabase(ctx, etat, journal);
      ...
      try { await page.waitForFunction(() => !!document.querySelector('input[type="password"]') || !!document.querySelector('nav, aside, [data-testid="composeur-a7"]') || !!document.querySelector('[data-testid="ecran-serveur-injoignable"]'), null, { timeout: scen.attenteMs || 20000 }); dureeAvantEcranMs = Date.now() - t0; } catch {}
      ...
      const m = await page.evaluate(() => {
        ...
        return {
          ...
          ecranReprise: !!document.querySelector('[data-testid="ecran-serveur-injoignable"]'), boutonReessayer: boutons.some((t) => /^Réessayer$/.test(t)), messageReprise: document.body.innerText.includes('MokNet est momentanément injoignable, veuillez réessayer.'),
      ...
      if (scen.rejeu) {
        await page.evaluate(([cle, val]) => { localStorage.setItem(cle, val); Object.defineProperty(document, 'visibilityState', { configurable: true, get: () => 'visible' }); document.dispatchEvent(new Event('visibilitychange', { bubbles: true })); }, [CLE, JSON.stringify(sessionLocale)]);
      ...
      if (scen.apres) {
        const a = scen.apres;
        if (a.delaiAvantMs) await page.waitForTimeout(a.delaiAvantMs);
        const t1 = Date.now();
        const clesAvant = await page.evaluate(() => { try { return Object.keys(localStorage).filter((k) => k !== '__banc_seme__'); } catch { return 'inaccessible'; } });
        etat.mode = a.mode;
        if (a.action === 'bouton') await page.click('button:has-text("Réessayer")', { timeout: 10000 });
        else if (a.evenement === 'visibilitychange') await page.evaluate(() => { Object.defineProperty(document, 'visibilityState', { configurable: true, get: () => 'visible' }); document.dispatchEvent(new Event('visibilitychange', { bubbles: true })); });
        else await page.evaluate((ev) => window.dispatchEvent(new Event(ev)), a.evenement || 'online');
        const cibleInterface = a.attendu === 'INTERFACE_INTERNE';
        try { await page.waitForFunction((cible) => cible ? !!document.querySelector('nav, aside, [data-testid="composeur-a7"]') : !!document.querySelector('input[type="password"]'), cibleInterface, { timeout: a.attenteMs || 20000 }); } catch {}
        if (!cibleInterface) await page.waitForTimeout(Math.min(a.attenteMs || 4000, 4000));
        await page.waitForTimeout(800);
        const r = await page.evaluate(() => ({ navigationInterne: !!document.querySelector('nav, aside'), composeur: !!document.querySelector('[data-testid="composeur-a7"], textarea[placeholder^="Quoi de neuf"]'), champMotDePasse: !!document.querySelector('input[type="password"]'), ecranReprise: !!document.querySelector('[data-testid="ecran-serveur-injoignable"]'), cles: (() => { try { return Object.keys(localStorage).filter((k) => k !== '__banc_seme__'); } catch { return 'inaccessible'; } })() }));
        r.verdictEcran = r.ecranReprise && !r.navigationInterne ? 'ECRAN_REPRISE' : (r.navigationInterne || r.composeur ? 'INTERFACE_INTERNE' : (r.champMotDePasse ? 'ECRAN_AUTHENTIFICATION' : 'INDETERMINE'));
        r.dureeMs = Date.now() - t1; r.retourReseauApresMs = a.delaiAvantMs || 0; r.clesAvant = clesAvant; r.declencheur = a.action === 'bouton' ? 'bouton Réessayer' : `événement ${a.evenement || 'online'}`; r.modeReseau = a.mode; r.attendu = a.attendu; r.conforme = r.verdictEcran === a.attendu;
        r.ressaisie = false; // le banc ne saisit jamais d'identifiants : toute interface obtenue l'est SANS ressaisie
        m.apresReprise = r;
        await page.screenshot({ path: path.join(OUT, `${TAG}-${sc}-${canal}-apres-reprise.png`), fullPage: false });
      }
