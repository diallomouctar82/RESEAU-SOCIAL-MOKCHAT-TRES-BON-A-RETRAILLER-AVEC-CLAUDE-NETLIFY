...
Feu vert écrit de la Direction vers 15:11 UTC ; `main` `1466525` inchangé depuis le dernier Green Gate ; PR #100 fusionnée en squash sur la tête exacte `a921a2f` → `main` `a615593` à **15:12:30 UTC**. `moknet.net` sert la nouvelle feuille depuis **15:13:19 UTC** (bundle `index-Bm6woHcd.js` inchangé : aucun JavaScript modifié ; ancien bundle `index-DnzvQMzK.js` → 404 ; `cache-control: max-age=0, must-revalidate`). Vérificateur `scripts/production-controlee/verif-page-servie.sh` : « conforme » (marqueurs `a7 (max-width: 270px)`, `(max-width: 344px)`, `grid-template-rows: auto 1fr`, resserrement du groupe, `overflow-wrap: anywhere` présents ; aucun seuil 300/360 px ; aucune règle `scroll-snap` / `mask-image` / `overflow-x` dans la bande). Green Gate vert sur `main` (run 33974090869). Miroir local ouvert dans Chromium (`production-fumee.json`) : racine montée sur ordinateur et téléphone, requêtes de conteneur `a7` 560/270 et `aurore` 800/480 analysées, deux replis `@supports`, bundle `index-Bm6woHcd.js`.

Limite honnête : le composeur est derrière l'authentification ; la page publique servie est l'écran de connexion. Les captures ordinateur et téléphone du composeur ci-dessus proviennent du harnais exécutant ce même code ; le contrôle visuel dans l'application authentifiée appartient à la Direction (ou à un compte de preuve créé sur demande, supprimé ensuite à zéro trace).
