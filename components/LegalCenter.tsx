...
export const LegalCenter: React.FC<LegalCenterProps> = ({ userProfile }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 animate-fade-up">
            {/* Header */}
            <div className="bg-slate-900 text-white p-8 shadow-lg">
                <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                        <div className="flex items-center gap-2 text-brand-300 font-bold uppercase text-xs tracking-wider mb-2">
                            <Scale size={14} /> Espace Juridique & Administratif
                        </div>
                        <h1 className="text-3xl font-bold">Legal Guardian</h1>
                    ...
                    <div className="flex gap-2 bg-white/10 p-1 rounded-xl backdrop-blur-sm">
                        <button 
                          ...
                          className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'dashboard' ? 'bg-brand-600 text-white shadow-md' : 'text-white hover:bg-white/10'}`}
                        >
                            Mes Procédures
                        </button>
                        <button 
                          onClick={() => setActiveTab('scanner')}
                          className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'scanner' ? 'bg-brand-600 text-white shadow-md' : 'text-white hover:bg-white/10'}`}
                        >
                            Scanner
                        </button>
                        <button 
                          onClick={() => setActiveTab('writer')}
                          className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'writer' ? 'bg-brand-600 text-white shadow-md' : 'text-white hover:bg-white/10'}`}
                        >
                            Rédiger
                        </button>
                    </div>
                ...
                <div className="max-w-5xl mx-auto">
                    ...
                    {activeTab === 'dashboard' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm col-span-full mb-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-orange-50 p-4 rounded-xl">
                                        <div className="text-2xl font-black text-orange-600">1</div>
                                    ...
                                    <div className="bg-purple-50 p-4 rounded-xl">
                                        ...
                                        <div className="text-xs text-purple-400 font-bold uppercase">Score Dossier</div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div key={proc.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
                                    ...
                                    <div className="p-6">
                                        <div className="flex justify-between items-start mb-3">
                                            ...
                                            {proc.status === 'blocked' && <AlertTriangle size={16} className="text-red-500" />}
                                            {proc.status === 'completed' && <CheckCircle size={16} className="text-green-500" />}
                                        </div>
                                        ...
                                        <div className="flex justify-between items-center text-xs text-slate-400 bg-slate-50 p-3 rounded-lg">
                                            <span className="flex items-center gap-1"><Clock size={12} /> {proc.deadline}</span>
                                            <button className="font-bold text-brand-600 hover:text-brand-700 flex items-center gap-1">
                            ...
                            {/* Add New */}
                            <div className="bg-slate-50 rounded-2xl border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 p-6 hover:bg-white hover:border-brand-300 transition-all cursor-pointer group min-h-[200px]">
                                <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center mb-3 shadow-sm group-hover:scale-110 transition-transform">
                                    <span className="text-2xl text-slate-300 group-hover:text-brand-500">+</span>
                                </div>
                            ...
                            <div className="space-y-6">
                                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                                    ...
                                    <div 
                                      ...
                                      className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:bg-slate-50 cursor-pointer transition-colors relative"
                                    >
                                        {scannedImage ? (
                                            <img src={scannedImage} className="max-h-64 mx-auto rounded shadow-sm" />
                                        ) : (
                                    ...
                                    <button 
                                        ...
                                        className={`w-full mt-4 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg ${!scannedImage || isScanning ? 'bg-slate-200 text-slate-400' : 'bg-brand-600 text-white hover:bg-brand-700'}`}
                                    >
                            ...
                            <div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button 
                                        ...
                                        className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg ${!writerSubject || isWriting ? 'bg-slate-200 text-slate-400' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                                    >
                                ...
                                {generatedLetter ? (
                                    <div className="animate-fade-up h-full flex flex-col">
                                        ...
                                        <div className="pt-6 mt-6 border-t border-slate-100 flex justify-end gap-3">
                                            <button className="px-4 py-2 text-slate-500 hover:text-slate-800 text-sm font-bold">Copier</button>
                                            <button className="px-4 py-2 bg-brand-600 text-white rounded-lg text-sm font-bold shadow hover:bg-brand-700">Télécharger PDF</button>
                                        </div>
                                    </div>
                                ) : (
