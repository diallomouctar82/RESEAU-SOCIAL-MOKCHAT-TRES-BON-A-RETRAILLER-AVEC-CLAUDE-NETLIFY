...
export const PointAToBPathway: React.FC<PointAToBPathwayProps> = ({
    ...
    return (
      <div className={`bg-white border border-slate-200/80 rounded-2xl p-4 shadow-sm transition-all hover:border-slate-300 ${className}`}>
        ...
        <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden flex">
          <div 
            className="bg-gradient-to-r from-slate-900 via-blue-700 to-blue-600 h-full rounded-full transition-all duration-700" 
  ...
  return (
    <div className={`bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8 shadow-sm relative overflow-hidden ${className}`}>
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-bl from-blue-500/5 via-indigo-500/5 to-transparent rounded-full pointer-events-none blur-2xl" />
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8 pb-6 border-b border-slate-100">
        <div>
          ...
          <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            ...
            <span className="text-blue-600">{destination.label}</span>
          </h3>
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <span className={`text-[11px] font-bold uppercase tracking-wider ${
                    isInProgress 
                      ? 'text-blue-700 bg-blue-100/80 px-2 py-0.5 rounded-full' 
                      : isCompleted 
                      ? 'text-slate-600' 
                      : 'text-slate-400'
                  }`}>
                ...
                {step.expertNote && isInProgress && (
                  <div className="mt-2.5 pt-2 border-t border-blue-200/60 text-[11px] text-blue-900/90 font-medium flex items-center gap-1.5">
