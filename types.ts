...
export interface UserProfile {
    ...
    role: UserRole;
    /**
     * Studio Avatar : offre du compte. Optionnel — un profil hérité sans plan
     * est un compte standard, jamais un Pro par accident (voir
     * `isProAccount`). Seul `'pro'` ouvre l'avatar personnel.
     */
    plan?: 'free' | 'pro';
    /** Studio Avatar : un compte suspendu n'a ni parcours ni offre affichée. */
    ...
    privacySettings: {
        ...
        /**
         * Avatar personnel du Studio Avatar (membres Pro) : photo, nom
         * d'appel, consentement daté et lignes de guidage. Rangé ici parce que
         * `privacy_settings` est une colonne RÉELLE de `profiles`, déjà
         * persistée par `GlobalContext.updateUserProfile` — aucune migration
         * inventée. Absent = le membre garde l'avatar par défaut défini par
         * l'Admin-Général. Type défini dans `services/studio/avatarStudio.ts`,
         * décrit ici pour éviter une dépendance circulaire types ↔ services.
         */
        avatarStudio?: {
            photoUrl: string;
            displayName: string;
...
export type StudioTab = 'image' | 'video' | 'vision' | 'avatar' | 'avatar_studio' | 'collaboration';
...
export interface PlatformDetailedModuleSettings {
  ...
  studio: {
    ...
    /**
     * Studio Avatar — photo d'avatar appliquée à TOUT nouveau compte, définie
     * par l'Admin-Général. `photoUrl` vide = aucun avatar imposé (les nouveaux
     * comptes affichent leurs initiales). Contrôlée par
     * `validateDefaultAvatarUrl` : le cliché de banque d'images hérité est
     * refusé, l'application le traitant déjà comme « avatar absent ».
     */
    defaultAvatar: {
      photoUrl: string;
      label: string;
