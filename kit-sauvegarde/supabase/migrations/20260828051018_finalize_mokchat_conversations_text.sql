...
declare
  ...
  conversation_key text; conversation_id uuid;
begin
  ...
  if not p_is_group then
    ...
    values(true,left(trim(p_title),160),current_user_id) returning id into conversation_id;
  end if;
  insert into public.conversation_participants(conversation_id,user_id,member_role)
  values(conversation_id,current_user_id,'owner') on conflict(conversation_id,user_id) do nothing;
  insert into public.conversation_participants(conversation_id,user_id,member_role)
  select conversation_id,m,'member' from unnest(members) m on conflict(conversation_id,user_id) do nothing;
